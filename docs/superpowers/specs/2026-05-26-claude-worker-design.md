...
| `infra/k8s/manifests/40-network-policy.yaml` | UPDATE — pipeline → claude-worker:8767; claude-worker → anthropic.com:443 + git forges |
...
## 9. Follow-up issues a criar pós-merge

1. **[FEATURE] `[d]` view: colunas avançadas por stage (timeout, retries, cost cap)** — espelha solicitação explícita do user durante o grilling. Não é pre-req da #309.
2. **[FEATURE] OAuth in-pod via port-forward para claude-login zero-touch** — habilita instalar claude-worker em cluster sem ter claude CLI no host. Boa pra Rancher remoto / CI.
3. **[FEATURE] claude-worker scale-up com RWX PVC** — multiple replicas. Hoje single-replica é gargalo se voc6e dispatcha 3+ tasks paralelas pra claude.
4. **[FEATURE] OTLP tracing no claude-worker** — span por dispatch, atributos task_id/duration/model/stage. Espelha #303 fase 4.
5. **[FEATURE] Múltiplos perfis claude (contas diferentes por stage)** — hoje single account. Útil se voc6e quer uma conta pra implement e outra pra review.
...
- Integração com Vault para Secret storage.
...
## 12. Open questions (resolver durante implementação)
