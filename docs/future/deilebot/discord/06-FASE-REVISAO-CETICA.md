...
| Diff implementado em `deilebot/providers/discord/` | 90min |
| Diff implementado em `deilebot/foundation/tools/` | 30min |
...
| S8 | memory.json 644 | 🟡 verificar `data/deilebot.sqlite` perms |
...
| B6 | `python -m discord_bot.bot` falso | 🟢 substituído por `python -m deilebot.cli run --provider discord` |
