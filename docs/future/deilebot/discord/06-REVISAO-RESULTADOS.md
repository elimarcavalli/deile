...
> Auto-revisão pelo agente implementador, branch deilebot. Live cenarios
...
| **S3** | Privilégio por display_name | 🟢 | `PermissionGate.is_owner` consulta apenas `bot_user_id` (`deilebot/foundation/permissions.py`); `IdentityResolver.resolve` mantém ULID estável independente de display_name |
...
| **S7** | nuke.py perigoso | 🟢 | archived; nada equivalente em deilebot |
| **S8** | memory.json 644 | 🟡 | SQLite criado pelo aiosqlite herda umask 022 (default); operador pode `chmod 600`. Documentado em `config/deilebot.example.yaml` (TODO) |
...
| **B6** | `python -m discord_bot.bot` falso | 🟢 | substituído por `python -m deilebot.cli run --provider discord`; `pyproject.toml` registra console_script `deilebot` |
...
| **A3** | Zero testes | 🟢 | 232 testes deilebot + 56 testes deile/core (M2) = 288 testes, todos verdes |
...
| **A5** | start.sh engole stderr | 🟢 | `cli.py` usa `setup_logging` JSON-structured, sem swallow; logs em `data/logs/deilebot.log` rotativos |
...
`deilebot/tests/e2e/discord/test_security_invariants.py` como
...
1. **Memória persistente cresce indefinidamente?** Não — `ConversationStore.purge_older_than(days)` + `SessionStore.purge_older_than(days)`. Default `data_retention_days=90`. Trigger manual via `/dlq purge` ou `python3 -m deilebot.cli sessions purge --older-than-days 30`.
