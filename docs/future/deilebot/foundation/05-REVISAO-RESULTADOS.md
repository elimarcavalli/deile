...
| F6 | Foundation não importa providers | 🟢 | `test_settings.py::TestNoProvidersImportInFoundation` faz grep recursivo e passa. `_testing.py` (que precisa importar `ProviderAdapter`) vive em `deilebot/_testing.py` (top-level), não em `foundation/`. |
...
- **Desvio**: spec sugere `_testing.py` em `foundation/`; movemos para `deilebot/_testing.py` para honrar F6 estritamente. Documentado neste arquivo e nos commits.
...
6. **Operador precisa de quê em prod?** snapshot `/metrics` (counters de inbound/agent/outbound + DLQ size + rate_limited), audit recente (`SELECT * FROM audit ORDER BY occurred_at DESC LIMIT 50`), DLQ count alerta `> 10`, log file `data/logs/deilebot.log`.
