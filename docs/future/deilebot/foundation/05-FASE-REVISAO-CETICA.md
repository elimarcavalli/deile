# Fase Revisão Cética — `deilebot/foundation/`
...
- [ ] Pacote `deilebot/providers/*` realmente vazio de imports em `deilebot/foundation/*`? (verificar via `grep -r "from deilebot.providers" deilebot/foundation/`)
- [ ] `pytest --cov=deilebot/foundation` ≥ 85%?
...
- [ ] `pytest deilebot/tests/e2e/ -v -m e2e` verde com custo < $0.05?
