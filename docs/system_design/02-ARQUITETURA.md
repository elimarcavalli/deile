...
| `orchestration/pipeline/` | Pipeline autônomo de issues/PRs/menções: `PipelineMonitor` (loop de polling; estágios `classify` → `review` → `implement` (+`resume`) → `pr_review` → `follow_ups`, mais `process_mentions` como roteador por papel); `PipelineImplementer` (estratégia plugável — `ClaudeImplementer` via `claude -p` em worktree **ou** `WorkerImplementer` que despacha ao `deile-worker` por HTTP, selecionada por `dispatch_mode`); `dispatch_resolver.py` (resolve qual worker atende cada stage — `resolve_stage_dispatcher`/`get_endpoint_for`; `VALID_DISPATCHERS` = workers núcleo `deile-worker`/`claude-worker` ∪ frota CLI **derivada do registro de adapters** `cli_adapters.ADAPTERS`, nunca hardcodada — Decisão #51); `cli_worker_scaler.py` (`ensure_replica` — escala on-demand 0→1 os workers da frota CLI, que nascem `replicas:0`, com cooldown anti-flapping, reusando o RBAC do pipeline pod); `ResumeTracker` (`resume_state.py` — cadência/fingerprint/tentativa por item, issue #254); `briefs.py` (templates de prompt: implement/review/review-only/address/mention + variantes de resume; **tooling-agnostic via placeholders `{forge_*_cmd}` preenchidos pelo `forge/cli_renderer`** — Decisão #41); `github_client.py` (shim deprecated que re-exporta de `forge/`, mantido por 1 release); `WorktreeManager` (`_ensure_forge_remote` + alias `github` para repos GH — Decisão #41), `ClaudeDispatcher` (`claude -p` subprocess), `DiscordNotifier`, `MonitorIdentity` (sharding hash-based), `LockFile` (PID lock), `ScheduleStore`/`Schedule`/`RecurringEntry`/`OneshotEntry` (scheduler YAML por monitor), `cron.py` (parser de expressões 5-field) |
...
## Camada de workers — frota de dispatch (`infra/k8s/`)

> Vive **fora** do pacote `deile/` (lado de deployment), mas é a contraparte de execução do `orchestration/pipeline/`: o pipeline despacha cada stage por HTTP para um pod-worker. Detalhe de containerização em [`14-CONTAINERIZACAO.md`](14-CONTAINERIZACAO.md); o contrato do componente plugável em [`04-MODELO-COMPONENTES.md`](04-MODELO-COMPONENTES.md).

A frota tem **dois workers núcleo** com servers dedicados — `deile-worker` (roda DEILE Python in-process via `wrapper.py worker`) e `claude-worker` (`claude -p` em worktree isolado, com OAuth) — e uma **frota CLI plugável** ao lado deles. Cada CLI de coding headless é plugado por um adapter; o server e a maquinaria genérica são compartilhados.

| Componente | Arquivo | Responsabilidade |
|---|---|---|
| Registro de adapters | `infra/k8s/cli_adapters/` | Pacote auto-descoberto: cada `<kind>.py` satisfaz o Protocol `CliAdapter` (`base.py`) e o `__init__.py` monta `ADAPTERS = {kind: adapter}` em import — a **fonte única de verdade da frota** (Decisão #51). Adicionar um worker = escrever um adapter; nenhum consumidor é editado |
| Núcleo agnóstico | `infra/k8s/_worker_core.py` | Maquinaria independente do CLI concreto, extraída do claude-worker: lease atômico multi-réplica, heartbeat, subprocess one-shot com persistência de stdout/stderr no PVC, middleware Bearer, validação de `task_id`, classificação de erro de provider (anti-sangria de custo, issue #445) |
| Server genérico | `infra/k8s/cli_worker_server.py` | Servidor HTTP que reusa `_worker_core` e delega ao adapter selecionado por `DEILE_CLI_WORKER_KIND` os pontos divergentes. Endpoints `POST /v1/dispatch`, `GET /v1/health`, `GET /v1/progress/{task_id}`, `GET /v1/models`. Gate pós-run (commit+push, ou test verde) por cima do `WorkResult.ok` do adapter — exit-code dos CLIs não é confiável |
| Resolução/scaling no pipeline | `deile/orchestration/pipeline/dispatch_resolver.py`, `cli_worker_scaler.py` | `dispatch_resolver` deriva `VALID_DISPATCHERS`/endpoints do registro de adapters (núcleo ∪ frota); `cli_worker_scaler` garante ≥1 réplica on-demand (frota nasce `replicas:0`) |
...
| `orchestration/pipeline/dispatch_resolver.py` → `cli_adapters.ADAPTERS` (lazy, em `infra/k8s/`) | Deriva a frota de workers válidos do registro — degrada para só os workers núcleo se o pacote estiver ausente |
