...
| Evidência | `infra/k8s/Dockerfile`; `infra/k8s/wrapper.py` (`_SENSITIVE_KEYS`, `_patch_deile_bootstrap`, `_pop_sensitive_keys`); `infra/k8s/manifests/00-namespace.yaml` (PSS labels); `infra/k8s/manifests/20-bot-deployment.yaml`, `infra/k8s/manifests/30-deile-job.yaml`, `infra/k8s/manifests/35-deile-interactive.yaml` (securityContext); `infra/k8s/manifests/40-network-policy.yaml` (5 policies); `infra/k8s/deploy.py` (orquestrador Python build/up/test/start/stop) |
...
> **SUPERSEDED por [Decisão #45](#decisão-45--brief-unificado-de-pr--worker-monta-work-list-pelo-estado-não-pelo-trigger).**
> A divisão por mode (`work_merge` / `review_only` / `address`) e o gate cross-tick por `~mention:processado` em assignee/reviewer foram removidos. O roteamento por papel para PRs colapsou em um único brief unificado que descobre o que fazer pelo estado real da PR (autor, HEAD vs último review, threads, comments dirigidos a mim). A persona `reviewer` para PR-scope, o roteamento de `issue + assignee/body → ~workflow:nova`, o caminho `comment em issue` e o quality-gate de arquitetura PERMANECEM válidos — só o sub-roteamento PR-scope foi unificado.

| Campo | Valor |
|---|---|
| Versão | V1 (SUPERSEDED por #45) |
...
## Decisão #41 — DEILE forge-agnóstico (GitHub + GitLab) via camada `deile/orchestration/forge/`

| Campo | Valor |
|---|---|
| Versão | V1 |
| Pilar dono | 02-Arquitetura, 04-Componentes, 08-Segurança, 09-Configuração, 14-Containerização |
| Decisão | Toda comunicação com o sistema de hosting (issues, PR/MR, labels, comentários, merge, CI status) passa por **`ForgeClient` (ABC)** em `deile/orchestration/forge/base.py`, implementada por **`GitHubForge`** (port 1:1 do `GitHubClient` legado, via `gh`) e **`GitLabForge`** (nova, via `glab` + REST v4). `IssueRef`/`PrRef` aceitam payloads dos dois forges (`.from_gh_json` / `.from_gl_json`); a string `"pr"` é o vocabulário interno para PR (GitHub) e MR (GitLab). O pipeline (`PipelineMonitor`) recebe `forge: ForgeClient`; `monitor.github` permanece como alias retroativo (`__getattr__`) por uma release. O legacy `pipeline/github_client.py` vira shim com `DeprecationWarning` re-exportando todos os símbolos. Os briefs do worker (`pipeline/briefs.py`) e os prompts do Claude (`pipeline/claude_dispatcher.py`) ficam tooling-agnostic: usam placeholders `{forge_*_cmd}` preenchidos por `forge/cli_renderer.py.render_brief_cmds(forge_config)` — o mesmo template renderiza `gh pr create` ou `glab mr create` conforme o forge. Detecção do forge é em camadas explícitas (`forge/detection.py`): override `DEILE_FORGE_KIND` → host parse (`github.com`/`gitlab.com` + hosts declarados em `DEILE_*_HOST`) → path heuristic (3+ segmentos → GitLab; 2 segmentos → GH para compat retroativa). `ForgeRouter` cacheia clients por `(host, project_path)` para sessões CLI multi-repo. |
| Evidência | `deile/orchestration/forge/{__init__,base,refs,url_parser,detection,cli_renderer,github_forge,gitlab_forge}.py`; `deile/orchestration/pipeline/{github_client,monitor,stages,briefs,claude_dispatcher,worktree_manager,implementer,constants}.py` (refatorados); `deile/config/settings.py` (chaves `forge_*`); `deile/security/secrets_scanner.py` (`SecretType.GITLAB_TOKEN` + 6 padrões `glpat-`/`gldt-`/`glptt-`/`glsoat-`/`GITLAB_TOKEN=`/catch-all); `infra/k8s/wrapper.py` (`_setup_forge_credentials` dual-token); `infra/k8s/Dockerfile` (camada `glab` v1.45.0); `infra/k8s/manifests/43-forge-tokens-secret.yaml` (doc); testes em `deile/tests/orchestration/forge/*.py` + `deile/tests/security/test_secrets_scanner_gitlab.py`. Issue #297. |
| Motivação | (1) Adoção em times GitLab self-hosted bloqueada — o pipeline autônomo é a entrega de mais alto valor do DEILE e o acoplamento direto a `gh` excluía metade do mercado endereçável; (2) GHES (GitHub Enterprise Server) também precisava de host customizável — a hardcoded `github.com` impedia até essa variante já existente do mesmo forge; (3) Os briefs eram a fronteira mais frágil — qualquer mudança de provider exigia reescrever 9 templates inteiros; centralizar comandos em `cli_renderer` é refator hexagonal puro (adapter no `forge/`, núcleo limpo nos `briefs/stages/monitor`); (4) Auth dual permite que UMA imagem `deile-stack:local` sirva os dois forges; o operador escolhe via Secret (sem rebuild). |
| Fora do escopo | Bitbucket / Gitea / Forgejo (camada projeta para acomodar, mas só GH+GL nesta entrega); OAuth interativo (auth continua por PAT estático, mesma postura GH atual); migração automática de issues/PRs entre forges; convergência de features Premium-only do GitLab (Required Approvals, Code Owners etc. — `ForgeClient` honra o que o forge faz, sem tentar normalizar comportamento); GUI/comando para alternar forge ativo durante sessão CLI (`ForgeRouter` resolve por URL/repo automaticamente); GitHub App auth; e2e contra GitLab real (aguarda credencial — esqueleto de teste pronto em `deile/tests/might/forge/` para o piloto). |
| Backwards-compat | 100% para callers existentes: o shim `pipeline/github_client.py` re-exporta cada símbolo legado (`GitHubClient = GitHubForge`); `monitor.github` continua funcionando via `__getattr__`; `_ensure_github_remote` é alias de `_ensure_forge_remote`; `_setup_gh_auth` / `_setup_git_credentials` são wrappers de `_setup_forge_credentials`; `resolve_pipeline_repo` delega a `resolve_forge_repo`; `DEILE_PIPELINE_REPO` continua aceito (cai pra default quando `forge_repo` vazio). Toda a suíte de testes 3547→3673 verde sem mudança nos arquivos de teste pré-existentes. |
| Honestidade | **Rate limit sleep ativo** e **HTTP probe opt-in (`DEILE_FORGE_PROBE=1`)** estão documentados nos docstrings/settings mas **não implementados** nesta entrega — o cliente lê e propaga erros de rate limit do `gh`/`glab`, mas não dorme até `reset` automaticamente. Implementar exige captura de headers HTTP via `--include` ou um cliente Python REST direto (refator não-trivial). Fica como issue derivada se o piloto GitLab esbarrar no limite (`gitlab.com`: 600 req/min/usuário). Para o piloto, o operador define `DEILE_PIPELINE_POLL_INTERVAL` mais conservador. |

---

## Decisão #44 — Painel de observabilidade live (chat-watch do claude-worker + status server do pipeline + 3 telas TUI)

| Campo | Valor |
|---|---|
| Versão | V1 |
| Pilar dono | 02-Arquitetura, 04-Componentes, 11-Observabilidade, 14-Containerização |
| Decisão | Em vez de inferir o estado do cluster por log-tailing (`kubectl logs deploy/deile-pipeline`), o painel TUI passa a consumir **estado estruturado** publicado por dois servidores HTTP. (1) **Novo módulo `infra/k8s/pipeline_status_server.py`** roda dentro do pod `deile-pipeline` na porta `:8768` expondo `/v1/pipeline-status*`: `status` (uptime/tick metrics/pod visibility), `backlog` (items elegíveis pro próximo tick), `recent` (eventos chronological), `ledger` (snapshot do `DispatchLedger`), `reaper-preview` e `force-tick`. Estado vive num singleton `PipelineStatusState` thread-safe que o monitor publica via `record_*`/`set_*`. (2) **Endpoints novos no `claude_worker_server.py`** (mantendo as 4 rotas existentes): `GET /v1/sessions` (resumo de TODAS tasks no PVC), `GET /v1/sessions/{id}/command` (comando + prompt + env REDACTED por regex `*_API_KEY`/`*_TOKEN`/`ANTHROPIC_AUTH_TOKEN`), `GET /v1/sessions/{id}/chat?tail=N` (JSONL parseado em turns estruturados), `GET /v1/sessions/{id}/stdout?tail_bytes=K`, `POST /v1/sessions/{id}/kill` (gated por confirm token `yes-task-<task_id_short>`), `DELETE /v1/sessions/{id}/cleanup` (refuses 409 em task viva). PID descoberto on-demand via `/proc` (helper `_find_claude_pid`) — sem corrida com persistência. (3) **Novo subpacote `deile/ui/panel/observability/`** com `ClaudeJsonlParser` (incremental, tolerante a JSON malformado, marca `tool_use` sem `tool_result` correspondente), `ClusterObservabilityClient` (aiohttp wrapper com timeouts curtos + `ApiError` fallback — painel nunca trava se um pod estiver down), e 3 screens Rich-based (`ClusterStatusScreen`/`LiveSessionScreen`/`HistoryScreen`) que renderizam adaptativamente a `console.width` (princípio 15 do pilar 03, sem `width=N` literal). Bearer auth reusada (mesmo padrão constant-time `hmac.compare_digest`). Painel legado continua funcionando — V1 convive. |
| Evidência | `infra/k8s/pipeline_status_server.py` (300+ linhas, novo); `infra/k8s/claude_worker_server.py` (6 endpoints novos + helpers `_redact_env`/`_find_claude_pid`/`_claude_jsonl_path`); `deile/ui/panel/observability/{__init__,jsonl_parser,client,screens}.py`; testes em `deile/tests/ui/test_claude_jsonl_parser.py` (14), `deile/tests/infrastructure/test_claude_worker_server.py` (17 novos), `deile/tests/orchestration/pipeline/test_pipeline_status_server.py` (15), `deile/tests/ui/test_observability_screens.py` (10), `deile/tests/ui/test_observability_client.py` (5). Issue #347. |
| Motivação | (1) Operador ficava 99% do tempo no escuro entre dispatches; depois de mergear 2 PRs o painel mostrava só `dispatch completed` 3 horas atrás (mix de fuso UTC/local não ajudava). Saber qual claude está rodando, qual o session_id, o que ele está executando neste segundo, e a conversa ao vivo exigia 9 comandos `kubectl exec` separados (documentados na issue). (2) Pilar 11 (observabilidade) já cobre OTLP enterprise, mas a peça **ao-vivo do trabalho-em-curso** estava faltando — OTLP é pra agregação cross-pod, painel é pra inspeção single-pod. (3) JSONL do `claude -p` **já existe** em `~/.claude/projects/<hash>/<sid>.jsonl` — o painel só precisa parsear, não recriar. (4) Polling 1Hz é OK pra V1 (streaming/SSE fica pra fase posterior); aiohttp com timeout 3-5s e `asyncio.gather` paraleliza os 3 fetches sem bloquear render. |
| Fora do escopo | WebSocket/SSE (V1 = polling); Grafana/Prometheus exporters (já cobertos pelo OTLP da decisão #39); histórico cross-pod (V1 mostra só o PVC atual — se recriado, histórico = 0); auth granular por usuário (Bearer único do worker permanece); edição de `session.json` via painel (read-only exceto kill/cleanup/force-tick); recreação visual de tool_result inline (markdown tables/imagens/diffs não re-renderizadas); manifests K8s NetworkPolicy/Service para o `:8768` (declarado mas não automatizado em `deploy.py` — operador aplica via `kubectl apply` ou em PR de follow-up). |

---

## Decisão #45 — Brief unificado de PR — worker monta work-list pelo estado, não pelo trigger

| Campo | Valor |
|---|---|
| Versão | V1 |
| Pilar dono | 05-Fluxo, 04-Componentes, 08-Segurança |
| Decisão | Brief unificado de PR — worker monta work-list pelo estado, não pelo trigger. Substitui os 3 briefs anteriores (`_WORKER_REVIEW_BRIEF` / `_WORKER_REVIEW_ONLY_BRIEF` / `_WORKER_PR_ADDRESS_BRIEF`) e o brief de resume (`_WORKER_REVIEW_RESUME_BRIEF`) por um único `_WORKER_PR_BRIEF` renderizado por `_render_worker_pr_unified_brief(repo, main, number, *, gh_login, forge)`. O trigger só serve pra apontar qual PR olhar; o brief único decide o que fazer pelo papel (autor / assignee / requested reviewer) + cobertura de HEAD vs último review + threads abertas + comments dirigidos a mim sem resposta. Auto-menção em comment não vira trigger (drop no collector quando `comment.author == gh_login`). Assignee/reviewer (sticky-PR) deixam de ser gateados por `~mention:processado` — o estado natural (HEAD igual, review APPROVED, threads vazias) já filtra; o brief comenta curto "já APPROVED em \<sha\>, sem novidade" e o pipeline marca o sticky-success com o marker para evitar churn redundante. Body-mention permanece gateado (corpo é estático). Autor humano disparando o brief: o brief é explícito — NUNCA dou push; só comento e devolvo assignment se aplicável. Resume é gratuito: o PASSO 0 lê `.deile-progress.md` se existir; um mesmo brief atende fresh e resume. |
| Evidência | `deile/orchestration/pipeline/briefs.py` (`_WORKER_PR_BRIEF`, `_render_worker_pr_unified_brief`); `deile/orchestration/pipeline/implementer.py` (`WorkerImplementer.review` + `WorkerImplementer.mention` em ramo `target_kind == "pr"`); `deile/orchestration/pipeline/stages.py` (collector drop self-comment + ungate sticky de `MENTION_DONE`; router colapsa em `pr_unified`; mark always); `deile/orchestration/forge/refs.py` (`MentionTrigger.trigger_author`); testes em `deile/tests/orchestration/pipeline/test_briefs.py` (`TestUnifiedPrBrief`), `test_mention_handling.py` (`TestProcessMentionsModeRouting`), `test_collector_drops_self_comment.py`, `test_collector_assignee_not_gated_by_mention_done.py`, `test_router_pr_always_uses_pr_unified_brief.py`, `test_brief_state_driven.py`, `test_brief_resumes_from_progress_md.py`, `test_mention_done_marked_always.py`, `test_no_self_handle_in_posted_comments.py`. |
| Motivação | (1) O pipeline recebia até 3 dispatches sequenciais redundantes na MESMA PR quando múltiplos triggers se acumulavam (assignee + reviewer + body + comment), cada um disparando um brief diferente — uma PR já APPROVED ainda recebia revisão completa quando um humano comentava `@deile-one Pode revisar?`. O princípio inegociável: a PR é o quadro; o worker abre o quadro, vê o que falta fazer, faz. (2) A regra antiga "não marcar `~mention:processado` em `review_only` pra deixar o assignee re-disparar" deixou de fazer sentido porque o brief unificado já cobre as duas pontas (reviewer-mode e assignee-mode) numa única passada — o próximo tick só precisa re-disparar se houve mudança REAL de estado (HEAD novo, thread nova), e isso já entra pelo trigger natural. (3) O guard anti-storm `review_only pr#X: worker reported ok but no review posted` deixa de ser necessário: o checkpoint do brief unificado exige um comment visível, então silent-failure clássico se torna detectável pelo próprio brief. (4) Auto-eco: o próprio DEILE postava um comment citando `@deile-one`, o que armava trigger no próximo tick e disparava trabalho redundante — o drop no collector encerra esse loop. |
| Fora do escopo | Mudar o caminho de **comment em issue** (segue como `comment` mode com brief context-rich); mudar o brief do `WorkerImplementer.implement` (mantém o brief de implementação fresh/resume separado — implementação não é "abra a PR e veja o que falta", é "implemente a issue do zero"); mudar o brief de classify/refine/decompose (não tocam o eixo "PR é o quadro"). Granularidade de `pr_reviewer_still_requested` (mantém disponível no `ForgeClient` para uso futuro — não é mais chamada por padrão). |

### Histórico

- **2026-05-29** — Decisão criada. Refator que substitui a Decisão #32 no eixo de PR-scope. Removidos: `_WORKER_REVIEW_BRIEF`, `_WORKER_REVIEW_RESUME_BRIEF`, `_WORKER_REVIEW_ONLY_BRIEF`, `_WORKER_PR_ADDRESS_BRIEF`, `_render_worker_review_brief`, `_render_worker_review_resume_brief`, `_render_worker_review_only_brief`, `_render_worker_pr_address_brief`, `_MENTION_REVIEWER_MODES`. Adicionados: `_WORKER_PR_BRIEF`, `_render_worker_pr_unified_brief`. Tabela de modes de mention agora binária (`pr_unified` para PR, `comment` para issue-comment).

---

## Decisão #46 — Resilência do pipeline: resume sob demanda, PVC auto-cleanup, brief sempre executa, backoff exponencial de auth

| Campo | Valor |
|---|---|
| Versão | V1 |
| Pilar dono | 05-Fluxo, 04-Componentes, 08-Segurança, 14-Containerização |
| Decisão | Sete correções coordenadas no pipeline para garantir o princípio "erros operacionais não travam trabalho": (1) **Resume sob demanda** — fresh dispatch é o default; resume só é tentado quando o caller passa `resume=True` E há entry no `DispatchLedger`. O brief unified já lê `.deile-progress.md` no PASSO 0, então fresh-com-contexto-natural cobre 95% dos casos sem o crescimento ilimitado do JSONL (visto 11M tokens em produção). Resume com sessão > 100K tokens é PROMOVIDO automaticamente para fresh em vez de rejeitado (removeu `RESUME_BUDGET_EXCEEDED` 413). (2) **PVC auto-cleanup** — `claude_worker_server` ganha startup hook + task periódica (1h) que varre `/home/claude/work` e remove workdirs sem `.lease.json` ou com heartbeat > 30min; trigger agressivo (10min TTL) quando uso > 1GB. Antes acumulou 122 workdirs / 1.9GB. (3) **HTTP 413 defense** — `_dispatch` cap `resume_block` em 50 KiB/campo e 100 KiB JSON total, com sentinela "TRUNCATED — read .deile-progress.md for full" quando estoura. (4) **Brief sempre executa o pedido** — removido o ramo "autor é HUMANO → NUNCA dou push"; agora o brief instrui: PR open → push direto na branch; PR merged/closed → branch derivada `auto/<orig>-followup-<sha>` + nova PR. (5) **Comment em PR usa brief unified** — `CommentRef` ganha campo `is_pr_comment: bool` (populado pelo forge via URL e `noteable_type`); `MentionTrigger.target_kind` retorna "pr" quando o flag é True; comments regulares em PR (que vinham via `/issues/comments` com `kind="issue"`) agora roteiam corretamente para `pr_unified`. (6) **Auto-renew OAuth sem operador** — `kubectl` adicionado ao Dockerfile; pipeline pod ganha `serviceAccountName: claude-creds-renewer` (RBAC mínimo, já existente em manifest 51) e `automountServiceAccountToken: true` para o `_claude_creds_refresh` poder chamar `kubectl exec` direto. (7) **Backoff exponencial de auth** — `PipelineMonitor` ganha `_auth_failures_by_target` e `_paused_until_ts`; ao receber `WORKER_AUTH_EXPIRED` em `pr:N`/`issue:N`, incrementa contador; após 3 falhas consecutivas, pausa o target por `min(2^count * 60, 1800)` segundos. Surtos curtos não bloqueiam deterministicamente; sucesso reseta tudo. |
| Evidência | `infra/k8s/claude_worker_server.py` (`_cleanup_stale_workspaces`, `_workspace_cleanup_loop`, startup hook na `build_app`; promoção automática de fresh em vez de `RESUME_BUDGET_EXCEEDED`); `deile/orchestration/pipeline/implementer.py` (`_dispatch` consulta ledger só quando `resume=True`; cap de `resume_block` por campo + JSON total); `deile/orchestration/pipeline/briefs.py` (`_WORKER_PR_BRIEF` reformulado com "REGRA DE EXECUÇÃO" substituindo "autor é HUMANO"); `deile/orchestration/forge/refs.py` (campo `is_pr_comment` em `CommentRef`; `MentionTrigger.target_kind` honra o flag); `deile/orchestration/forge/github_forge.py` + `gitlab_forge.py` (populam `is_pr_comment` via URL/`noteable_type`); `deile/orchestration/pipeline/stages.py` (`_AUTH_BACKOFF_THRESHOLD`, `is_target_auth_paused`, `record_auth_failure_and_maybe_pause`, `reset_auth_failures`; chamadas inline no implement e review paths); `deile/orchestration/pipeline/monitor.py` (dicts `_auth_failures_by_target` e `_paused_until_ts`); `Dockerfile` (instala `kubectl` v1.31.4 em layer separada com SHA implícito do release do K8s); `infra/k8s/manifests/46-deile-pipeline-deployment.yaml` (SA + token mount); testes em `deile/tests/orchestration/pipeline/test_backoff_auth_expired.py`, `test_collector_pr_comment_routes_to_pr_unified.py`, `test_brief_state_driven.py` (atualizado), `test_briefs.py` (atualizado), `test_worker_implementer_resume.py` (atualizado). |
| Motivação | Testes empíricos pós-Decisão #45 expuseram 7 modos de falha onde erros operacionais bloqueavam trabalho legítimo. (1) Sessão JSONL acumulou 11M tokens — resume rejeitado, mas o JSONL é só artefato de transporte; o estado real está em `.deile-progress.md` + comments. (2) PVC encheu de workdirs órfãos sem reaper — cleanup só rodava no shutdown gracioso. (3) PR comments do humano ficavam em `kind="issue"` e caíam em brief legacy, ignorando que estavam em PR. (4) Brief tinha cláusula "autor humano → ignora push", o que fazia DEILE desistir de pedidos legítimos. (5) `WORKER_AUTH_EXPIRED` bloqueava na primeira falha mesmo quando o token estava sendo refrescado in-pod (refresh leva 5-30s) — backoff exponencial dá margem. (6) Pipeline pod não tinha kubectl → auto-renew silenciosamente quebrado. (7) HTTP 413 nascia de `resume_block` sem cap em campos string. |
| Fora do escopo | Reescrever o reaper de leases (mecanismo separado em #309); substituir `kubectl exec` por kube API client puro (futuro, exigiria reescrever todo `_claude_creds_refresh`); cleanup do registry/state files (escopo da issue #303). Implementação Kubernetes API in-cluster avaliada e rejeitada: adicionar kubectl ao image é simpler e usa código existente. |

### Histórico

- **2026-05-29** — Decisão criada. Sete correções aplicadas num único PR (`feat/pipeline-resilience-fixes`).
...
## Decisão #50 — OpenRouter como provider unificador (uma chave → todos os modelos)

| Campo | Valor |
|---|---|
| Versão | V1 |
| Pilar dono | 07-Integrações LLM, 04-Componentes, 09-Configuração, 14-Containerização, 08-Segurança |
| Decisão | OpenRouter (`https://openrouter.ai/api/v1`) é registrado como mais um provider do router multi-provider, reusando o adapter OpenAI existente — `OpenRouterProvider` é uma subclasse fina de `OpenAIProvider` (mesmo molde de `DeepSeekProvider`). O adapter OpenAI **já** parametriza `base_url` (de `ProviderConfig.base_url`) e `default_headers` (de `ProviderConfig.sdk_kwargs`), confirmado contra a doc do SDK `openai-python` (`AsyncOpenAI(base_url=..., default_headers=...)`) — nenhuma generalização foi necessária (OR1). O provider só registra quando `OPENROUTER_API_KEY` está presente (condicional, como os demais — OR3). **Custo (OR5):** o `OpenRouterProvider` envia `extra_body={"usage": {"include": true}}` em toda chamada e lê o `usage.cost` (custo cobrado, em créditos USD) da resposta como `cost_estimate` **autoritativo**, com a tabela estática do `model_providers.yaml` apenas como **fallback** quando a API não reporta custo — isso elimina o undercount sistemático que uma tabela fixa causaria (o OpenRouter aplica markup e roteia upstream dinamicamente). O seam é o par de hooks `_provider_extra_body()` (default `{}`) + `_stamp_reported_cost(usage, response)` (default no-op) no `OpenAIProvider`, chamados nos três caminhos (`generate`/`chat_with_tools`/`generate_stream`) antes de `estimate_cost`. **Slug com `/` (Part-3a):** `model_id` segue o formato `vendor/model` do OpenRouter (`openrouter:anthropic/claude-sonnet-4.6`); o validador `_MODEL_SLUG_RE` foi alargado para `^[a-z][a-z0-9_-]*:[a-z0-9._/-]+$` (aceita `/` no lado do modelo) em **ambas** as cópias gêmeas (`settings.py` + `deile_worker_client.py`, mantendo o "keep in sync"). **Circuit breaker (Part-3c):** o breaker do `TierRouter` é keyed por `provider_id` (string), então `openrouter` é coberto automaticamente para 429/503 — sem mudança no breaker. **Secret/propagação (OR4):** `OPENROUTER_API_KEY` entra em `LLM_KEYS` (fonte única do scan do `.env` em `deploy.py`), propagando para os Secrets `deile-secrets`+`bot-secrets` via `**llm`; entra em `_SENSITIVE_KEYS` do `wrapper.py` (pop pós-bootstrap, paridade de segurança com as outras keys); e ganha paridade na CLI `create-namespace` (`--openrouter-key`). O manifest do worker **não** muda — as keys são montadas como arquivos via `_load_secret_files` genérico. **Painel (OR6):** os modelos `openrouter:*` aparecem no picker per-stage automaticamente, pois o `ModelsProvider` do painel lê todos os modelos do `model_providers.yaml` — zero mudança em `_panel.py`. **Privacidade (Part-3d):** o OpenRouter roteia o prompt para provedores terceiros (Anthropic, OpenAI, Google, DeepSeek, Qwen…) — um hop adicional; aceitável para código do projeto, documentado em `.env.example` e no YAML. |
| Evidência | `deile/core/models/openrouter_provider.py` (subclasse; `_provider_extra_body`, `_stamp_reported_cost`, `_reported_cost_from`, `estimate_cost` preferindo `reported_cost_usd`); `deile/core/models/openai_provider.py` (hooks `_provider_extra_body`/`_stamp_reported_cost` + wiring nos 3 caminhos; acumulação de `total_reported_cost` em `chat_with_tools`); `deile/core/models/bootstrap.py` (`_PROVIDER_CLASSES["openrouter"]`); `deile/config/model_providers.yaml` (`providers.openrouter` + 6 modelos curados + budget per-provider); `deile/config/settings.py` + `deile/infrastructure/deile_worker_client.py` (regex `_MODEL_SLUG_RE` com `/`); `infra/k8s/deploy.py` (`LLM_KEYS`, `CreateNamespaceConfig.openrouter_key`, `--openrouter-key`); `infra/k8s/wrapper.py` (`_SENSITIVE_KEYS`); `.env.example`; testes: `deile/tests/test_openrouter_provider.py`, `deile/tests/infra/test_openrouter_deploy_panel.py`, `deile/tests/config/test_settings_pipeline_models.py` (slug `/`), `deile/tests/infrastructure/test_dispatch_payload_preferred_model.py` (slug `/` no wire); smoke live em `deile/tests/might/llm_validation/validate_openrouter.py` (gated `OPENROUTER_LIVE=1`). Extraído do plano `docs/superpowers/plans/2026-06-06-openrouter-deile-integration.md`. |
| Motivação | A cobrança separada de uso programático da Anthropic (15/jun/2026) torna urgente rotear o grosso do pipeline para modelos baratos. OpenRouter dá DeepSeek/Qwen/Gemini Flash sob a mesma chave/billing, com fallback e teto de gasto por chave, reduzindo o egress da frota a um host só (`openrouter.ai`). Reusar o adapter OpenAI (em vez de um adapter novo) mantém a superfície mínima — só identidade + custo + (config) mudam. Preferir o custo reportado pela API evita repetir o histórico de undercount com Claude. |
| Fora do escopo | Sidecar credential-proxy para o claude-worker (issue separada, ortogonal); roteamento dinâmico/fallback automático entre upstreams do OpenRouter (`models[]`/`route` no body) — V1 fixa um upstream por slug; per-provider budget tuning fino (valores iniciais 20/200 USD são placeholders conservadores); o caminho `claude -p` do claude-worker (este provider serve o `deile-worker` in-process + CLI local, não o subprocess `claude`). |

### Histórico

- **2026-06-07** — Decisão criada. Adapter OpenAI confirmado parametrizável via context7 (`/openai/openai-python`) antes de implementar — `base_url` + `default_headers` já suportados, OR1 virou config-only. Slug regex alargado para `/` nas duas cópias gêmeas. Custo autoritativo via `usage.include=true`.

---

## Decisão #51 — Frota multi-CLI: workers plugáveis via registro de adapters (Antigravity GATED)

| Campo | Valor |
|---|---|
| Versão | V1 |
| Pilar dono | 02-Arquitetura, 04-Componentes, 14-Containerização, 08-Segurança |
| Decisão | O padrão `claude-worker` é generalizado num **framework de N workers CLI plugáveis**: cada CLI de coding headless (opencode, codex, qwen, aider, goose, …) é plugado por **um** adapter em `infra/k8s/cli_adapters/<kind>.py` que satisfaz o Protocol `CliAdapter` (`build_argv`/`env_overlay`/`parse_output`/`list_models` + metadados `kind`/`default_port`/`auth_mode`/`supports_resume`/`supports_reasoning`/`git_strategy`/`auth_env_keys`/`egress_hosts`/`writable_dirs`/`oauth`). O **registro auto-descoberto** (`cli_adapters/__init__.py`) escaneia o pacote em import e monta `ADAPTERS = {kind: adapter}` — a **fonte única de verdade da frota**: `dispatch_resolver` deriva `VALID_DISPATCHERS` daqui, o painel deriva a lista de workers, a geração de manifest/NetworkPolicy lê os metadados. **Adicionar um worker = escrever um adapter; nenhum consumidor é editado.** O servidor genérico `cli_worker_server.py` reaproveita toda a maquinaria agnóstica do `_worker_core` (lease/heartbeat/subprocess one-shot/HTTP bearer/cleanup/gate pós-run de commit+push+test) e delega ao adapter só os cinco pontos que divergem. **Gate de sucesso (§1.6):** `WorkResult.ok = adapter.parse_output(...).ok AND wrapper_gate()` — exit-code dos CLIs não é confiável; o gate pós-run (commit novo + push, ou test verde quando o brief exige) é a autoridade final. **Git strategy:** `brief_driven` (o brief instrui `git add/commit/push`; wrapper valida) para opencode/codex/qwen/goose; `cli_autocommit` (o CLI commita, wrapper só pusha+testa) só para o **aider** (com `--no-attribute-*` pela regra do projeto: sem "(aider)"/Co-Authored-By). **Auth (§1.11):** dois modos — `env` (API key, default, não expira; preferido para automação) e `oauth_file` (cred OAuth mountada via `<kind>-login`, opt-in). codex declara **OpenAI direto** (`wire_api=responses` exigido pelo Codex inviabiliza a maioria dos modelos do OpenRouter) com `OPENAI_API_KEY` (env) + OAuth ChatGPT opt-in (`DEILE_CODEX_AUTH=oauth`); qwen via tríade `OPENAI_*` (Dashscope/OpenRouter); aider/goose via OpenRouter. **Decisões por adapter validadas contra a doc oficial via context7** (codex `codex exec --json --skip-git-repo-check`; aider `--message-file --yes-always --auto-commits`; qwen `-p --yolo --output-format json`; goose `goose run --no-session --output-format json --max-turns` + `GOOSE_MODE=auto`/`GOOSE_DISABLE_KEYRING=1`), nunca de memória — o smoke pré-flight contra o binário pinado (Fase C/D sub-passo .0) confirma os flags antes do E2E. **Antigravity GATED:** `cli_adapters/antigravity.py` existe como **gate documentado** — NÃO exporta `ADAPTER`/`get_adapter` (só a classe-rascunho `_AntigravityAdapterDraft` + sentinela `ANTIGRAVITY_GATED=True`), então o auto-discovery o **ignora** e `antigravity-worker` não vira dispatcher, não aparece no painel e não quebra o `k8s up`. Motivo: closed-source, auth headless por API-key não suportada no consumer (issue oficial #78 aberta), Google-locked, login default via keyring (hostil a `readOnlyRootFilesystem`). Só sai do gate quando o spike (Fase E1) provar **uma** rota de auth headless no pod — preferência: (1) Vertex/Gemini Enterprise service-account JSON > (2) OAuth file mountável > (3) keyring (inviável). Enquanto fechado, Gemini é servido via OpenRouter (`openrouter/google/gemini-*`) ou via `deile-worker` (provider `google`). **Invariante de continuidade:** nenhum worker da frota é obrigatório para o `k8s up` subir — workers nascem `replicas:0`, instalados/escalados on-demand (como o `claude-login`); o `claude-worker` permanece intocado, um entre vários. |
| Evidência | `infra/k8s/cli_adapters/base.py` (Protocol `CliAdapter` + `WorkResult`/`ResumeCtx`/`ModelInfo`/`OAuthSpec` + `BaseCliAdapter`); `infra/k8s/cli_adapters/__init__.py` (auto-discovery `_discover`/`reload_adapters`/`get_adapter`, `ADAPTERS`); adapters `opencode.py`/`codex.py`/`qwen.py`/`aider.py`/`goose.py`; gate `antigravity.py` (`ANTIGRAVITY_GATED`, `_AntigravityAdapterDraft` não-instanciado); `infra/k8s/_worker_core.py` (core agnóstico extraído do claude-worker); `infra/k8s/cli_worker_server.py` (server genérico); `deile/orchestration/pipeline/dispatch_resolver.py` (`get_valid_dispatchers`/`get_endpoint_for` derivados do registro). Testes: `deile/tests/infra/test_cli_adapter_registry.py`, `test_worker_registry_drives_everything.py` (anti-hardcode/derivação), `test_opencode_adapter.py`, `test_codex_adapter.py`, `test_qwen_adapter.py`, `test_aider_adapter.py`, `test_goose_adapter.py`, `test_antigravity_gate.py` (gate fechado). Extraído do plano `docs/superpowers/plans/2026-06-06-multi-cli-worker-fleet.md`. |
| Motivação | A cobrança separada de uso programático da Anthropic (15/jun/2026) torna a dependência de um único worker (`claude-worker`) um gargalo de custo/quota. Tornar "adicionar/rotear worker" uma operação trivial e barata — redistribuir o grosso do pipeline para CLIs/providers baratos (DeepSeek/Qwen/Gemini via OpenRouter) e reservar claude/codex para tarefas premium — neutraliza o lock de preço. Derivar tudo do registro de adapters elimina o anti-padrão de listas-de-workers hardcodadas em N consumidores. |
| Fora do escopo | E2E ponta-a-ponta de cada worker (Fase C4/D — smoke contra binário real + PR aberta); imagens Docker por `WORKER_KIND` + manifests/NetworkPolicy gerados (Fase C/D); o spike do Antigravity (Fase E1) e o adapter funcional (Fase E2, só se o spike passar); `deploy.py gen-worker`/`<kind>-login`/scale-to-zero on-demand (Fases A5/A6/B5); painel `DispatchMatrixView` com picker de modelo via `GET /v1/models` (Fase F1). Esta decisão cobre o framework + os adapters mock-level (build_argv/parse_output/list_models/metadados) com testes unitários. |

### Histórico

- **2026-06-07** — Decisão criada (Fases 2-5 do plano multi-CLI). Framework `CliAdapter` + auto-discovery + core agnóstico + server genérico (Fases A/2) e adapters opencode (Fase 4) → codex/qwen/aider/goose (Fase 5, mock-level). Antigravity registrado como gate documentado (não-implementado) por inviabilidade de auth headless confirmada na pesquisa. Flags de cada CLI validados contra a doc oficial via context7 antes de codificar.
