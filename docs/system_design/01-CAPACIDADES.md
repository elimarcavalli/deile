...
| Frota de CLI workers plugáveis, roteável por estágio | Além de `deile-worker` e `claude-worker`, o pipeline despacha trabalho para uma frota de workers CLI plugáveis (cada um um Pod com Service próprio). Cada estágio (`classify`/`refine`/`implement`/`pr_review`/`follow_ups`) escolhe independentemente o worker, o modelo e o esforço de raciocínio. Adicionar um worker é escrever um adapter — nenhum consumidor é editado (registro como fonte única, `get_valid_dispatchers()` em `dispatch_resolver.py`). Resolução por `DEILE_PIPELINE_DISPATCH_<STAGE>` > global `DEILE_PIPELINE_DISPATCH_MODE` (default `deile-worker`) — ver Decisão #51 |
| Resume nativo por worker + custo auditável | Cada worker da frota retoma a sessão nativa no mesmo workdir em vez de re-gastar tokens; erro de provider (402/429/insufficient) é classificado INCOMPLETO para o pipeline retomar. O custo de cada sessão é colhido para um ledger durável antes de o log volumoso ser podado, lido depois pela auditoria da frota — ver Decisão #51 |
