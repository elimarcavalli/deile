...
| 43 | `claude-worker` pod paralelo ao `deile-worker` para dispatch de `claude -p` em worktrees isolados. Per-stage routing via `dispatch_resolver` em `deile/orchestration/pipeline/` (espelha decisão #41 dos per-stage models): env var per-stage `DEILE_PIPELINE_DISPATCH_<STAGE>` + global `DEILE_PIPELINE_DISPATCH_MODE` (default `deile-worker`). View unificada `[d]` (`DispatchMatrixView`) substitui `[d]` global da PR #330 + `[M]` per-stage do #305 — matriz N+1 stages × 2 colunas (Worker + Model), com `[L]` switch login e `[I]` install se ausente. Credentials (**atualizado pela Decisão #52**): `CLAUDE_CODE_OAUTH_TOKEN` (token ~1 ano via `claude setup-token`) injetado como env var via Secret `claude-credentials` — sem initContainer, sem `credentials.json`, sem CronJob de renovação. NetworkPolicy ingress só do `deile-pipeline`; egress 443 whitelisted (`api.anthropic.com`, `github.com`, `gitlab.com`) + DNS; granularidade de repo via ConfigMap `claude-worker-allowed-repos` enforcado no `wrapper.py` **e reverificado por request antes do clone** (`_worker_core.check_repo_allowed`, issue #639). Service `:8767` expõe `POST /v1/dispatch`, `GET /v1/health`, `GET /v1/progress/{task_id}`. `deploy.py k8s claude-setup-token` (preferido; `claude-login` legado). Threat model documentado em spec §7 (gap conhecido: exfiltração via canais legítimos — git push em repo whitelisted, headers HTTP — mitigada V1 por audit logging em `/v1/dispatch` + pattern detection); FUs prioritárias: sidecar credential proxy + integração Vault — issue #309 | V1 | Containerização (14), Componentes (04), Princípios (03) |
...
| 45 | Brief unificado de PR — worker monta work-list pelo estado, não pelo trigger. Substitui os 3 briefs anteriores (`_WORKER_REVIEW_BRIEF` / `_WORKER_REVIEW_ONLY_BRIEF` / `_WORKER_PR_ADDRESS_BRIEF`) e o brief de resume (`_WORKER_REVIEW_RESUME_BRIEF`) por um único `_WORKER_PR_BRIEF`. Trigger só serve pra apontar QUAL PR olhar; o brief único decide o que fazer pelo papel (autor/assignee/requested reviewer) + cobertura de HEAD vs último review + threads abertas + comments dirigidos a mim sem resposta. Auto-menção em comment não vira trigger (drop no collector quando `comment.author == gh_login`). Assignee/reviewer (sticky-PR) deixam de ser gateados por `~mention:processado` — o estado natural já filtra; sticky-success sempre marca o marker para evitar churn. Resume é gratuito (PASSO 0 lê `.deile-progress.md`). SUPERSEDES o eixo PR-scope da Decisão #32. **Atualizado pela Decisão #46:** o ramo "autor humano → NUNCA dou push" foi substituído por "execute o pedido sempre (push em PR open, branch derivada em PR merged)". | V1 | Fluxo (05), Componentes (04), Segurança (08) |
| 46 | Resilência do pipeline: (a) **resume sob demanda** — fresh dispatch é o default; resume só com `resume=True` E entry no `DispatchLedger`; resume com sessão > 100K tokens promove para fresh em vez de rejeitar. (b) **PVC auto-cleanup** — `claude_worker_server` ganha startup hook + task periódica (1h) que remove workdirs sem `.lease.json` ou com heartbeat > 30min; trigger agressivo (10min TTL) quando uso > 1GB. (c) **HTTP 413 defense** — `_dispatch` cap `resume_block` em 50 KiB/campo + 100 KiB JSON total com sentinela. (d) **Brief sempre executa o pedido** — `_WORKER_PR_BRIEF` substitui "autor humano → NUNCA dou push" por "PR open → push direto; PR merged/closed → branch derivada + nova PR". (e) **Comment em PR usa brief unified** — `CommentRef` ganha campo `is_pr_comment`; comments regulares em PR (que vinham via `/issues/comments` com `kind="issue"`) agora roteiam corretamente para `pr_unified` em vez do brief legacy. (f) **Auto-renew OAuth no pipeline pod** — `kubectl` adicionado ao Dockerfile; pipeline pod ganha `serviceAccountName: claude-creds-renewer` + `automountServiceAccountToken: true` (RBAC já existia em manifest 51). (g) **Backoff exponencial de auth** — `PipelineMonitor._auth_failures_by_target` + `_paused_until_ts`; após 3 falhas `WORKER_AUTH_EXPIRED` consecutivas, pausa target por `min(2^count*60, 1800)`s. Surtos curtos (refresh in-pod) não bloqueiam; sucesso reseta. | V1 patch | Fluxo (05), Componentes (04), Segurança (08), Containerização (14) |
| 47 | **Reasoning effort por worker × stage** (espelha os per-stage models da #41): cada etapa do pipeline e o DEILE CLI configuram o esforço de raciocínio. Vocabulário Claude Code (`low/medium/high/xhigh/max/ultracode/auto`) para claude-worker E deile-worker+anthropic; níveis oficiais por provider para os demais (openai `reasoning_effort`, gemini `thinking_config`, deepseek `reasoning_effort`+`thinking`). Fonte única em `deile/core/models/reasoning.py` (vocabulário + mapeamento → parâmetro nativo, fail-open) + `reasoning_resolver.py`. Viaja em `DispatchPayload.preferred_reasoning`; deile-worker injeta em `context_data["reasoning_effort"]` (provider traduz via `extra_body`/`ThinkingConfig`), claude-worker passa a `claude --effort`. Persistência tripla (env `DEILE_PIPELINE_REASONING_<STAGE>`/`DEILE_REASONING_EFFORT`, settings.json `pipeline.reasoning.*`/`model.reasoning_effort`, flag `--reasoning`). UI: coluna **Reasoning** na `DispatchMatrixView` (`[d]`) + comando `/reasoning` no CLI. | V1 | Integrações LLM (07), Componentes (04), Configuração (09), Containerização (14) |
| 48 | Adapter OTLP-traces para eventos dispatch.*/git.*/forge.*: `dispatch_schema.py` (dataclasses `frozen=True` com atributos canônicos, `SCHEMA_VERSION=1.0.0`, `get_pod_metadata()`) + `dispatch_export.py` (10 funções `emit_*` thread-safe). Root span `deile.dispatch` com span events para o ciclo dispatch; child spans para `git.commit`/`git.push`/`forge.pr_open`/`forge.pr_review`. Redact automático de tokens. Drop counter throttled ≤1×/60s. Zero leitura de `os.environ`. Extensão da Decisão #39; integração com `dispatch_logger` (#435) quando aquela PR mergear. | V1 | Observabilidade (11), Arquitetura (02), Segurança (08) |
| 49 | `deile-monitor` vira 3º alvo de dispatch on-demand: `monitor_command_server.py` (processo principal, agenda o tick determinístico como subprocess) expõe `:8769` (status sem-LLM, ordens `pause`/`resume`/`ack`/`force-tick`, Q&A read-only via persona `monitor_qa`); deilebot ganha cog `/monitor` + auto-route de perguntas de cluster + conserto do `POST /v1/notify`; Secret `monitor-bearer` compartilhado bot↔monitor. | V1 | Containerização (14), Componentes (04), Segurança (08), Fluxo (05) |
| 50 | OpenRouter como provider unificador (uma chave → todos os modelos): `OpenRouterProvider` subclasse fina de `OpenAIProvider` (molde do DeepSeek), reusa `base_url`+`default_headers` do adapter OpenAI (sem generalização — OR1 config-only); registro condicional por `OPENROUTER_API_KEY`; custo AUTORITATIVO via `usage.cost` da resposta (`extra_body={"usage":{"include":true}}`) com tabela do YAML como fallback (sem undercount); slug regex alargado p/ aceitar `/` no model_id (`openrouter:vendor/model`) nas duas cópias gêmeas; circuit breaker keyed por `provider_id` cobre 429/503 sem mudança; `OPENROUTER_API_KEY` em `LLM_KEYS`/`_SENSITIVE_KEYS`/CLI create-namespace; picker do painel lista `openrouter:*` automaticamente; privacidade (hop a terceiros) documentada. | V1 | Integrações LLM (07), Componentes (04), Configuração (09), Containerização (14), Segurança (08) |
| 51 | Frota multi-CLI: workers plugáveis via registro de adapters (`infra/k8s/cli_adapters/<kind>.py` satisfazendo o Protocol `CliAdapter` → auto-discovery monta `ADAPTERS`, fonte única que dirige `dispatch_resolver`/painel/manifests/NetworkPolicy). Server genérico `cli_worker_server` reusa `_worker_core`; gate pós-run commit+push+test (exit-code dos CLIs não basta). Adapters opencode/codex/qwen/aider/goose validados contra a doc oficial via context7 antes de codificar; aider é `cli_autocommit` (`--no-attribute-*`), demais `brief_driven`. Auth `env` (default, API key) ou `oauth_file` (opt-in); codex preferindo OpenAI direto (`wire_api=responses`). **Antigravity GATED** — `cli_adapters/antigravity.py` é gate documentado (não exporta `ADAPTER` → auto-discovery ignora; sentinela `ANTIGRAVITY_GATED=True`): closed-source + auth headless inviável no consumer (issue #78), só sai do gate com spike provando Vertex SA. Nenhum worker é obrigatório p/ o `k8s up`; claude-worker intocado, um entre vários. | V1 | Arquitetura (02), Componentes (04), Containerização (14), Segurança (08) |
| 52 | Auth do `claude-worker` via `claude setup-token` (token ~1 ano em `CLAUDE_CODE_OAUTH_TOKEN`, injetado como env var via Secret K8s) substitui o OAuth de ~8h (credentials.json + flock + initContainer `bootstrap-creds` + CronJob de renovação 4h). Verb `deploy.py k8s claude-setup-token` (+ hotkey `[T]` no painel); `claude-login`/`claude-renew` DEPRECATED. Fix de segurança: o strip de `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` do env do subprocess (venciam o token na precedência) agora é real, não só docstring; guard anti-`--bare` (bare mode não lê o token). Billing: a partir de 15/jun/2026, uso via `claude -p` em planos de assinatura consome crédito mensal de Agent SDK separado do interativo — issue #603 | V1 patch | Containerização (14), Segurança (08), Componentes (04) |
