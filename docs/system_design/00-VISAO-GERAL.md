...
| 40 | UI resize-adaptativa: substituir desenhos manuais de caixa com largura derivada de texto (`╔══╗` calculado por `max(len(...))`) por primitivas Rich (`Panel`, `Rule`) que consultam `console.width` lazy em cada render. Aceita explicitamente a limitação fundamental: conteúdo já no scrollback NÃO reflowa (texto ANSI estático no buffer do emulador). Rejeita `SIGWINCH` (não cross-platform), `clear()+replay` (destrói scrollback) e `screen=True` (elimina scrollback) — issue #307 | V1 patch | Princípios (03), Componentes (04) |
