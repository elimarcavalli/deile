...
**Pipeline label state machine** (issues): `~workflow:nova` → `~workflow:em_revisao` → `~workflow:revisada` → `~workflow:em_implementacao` → `~workflow:em_pr`. PRs: `~review:pendente` → `~review:em_andamento` → `~review:concluida`. Locks: `~batch:<sha8>` (claim — **only applied when `shard_count>1`; a single monitor skips it**, so no add/remove churn), `~by:<id>` (owner). Markers: `~workflow:bloqueada` (hard block), `~mention:processado` (mention/assignment already handled).
...
- **Mention/assignment routing (issues #253/#261):** `process_mentions` is a router. **Issue + assignee/body-mention** → injects `~workflow:nova` (pipeline takes over, with resume + `auto/issue-N` branch). **PR + assignee** → review + resolve threads + merge. **PR + requested-reviewer (only)** → reviews and **assigns the author back, never merges**. **PR/issue + comment** → does what the comment asks. Sticky triggers are marked `~mention:processado` so they don't re-fire each tick (remove it to re-handle). The PR review/merge work runs under the **`reviewer` persona** (`personas/instructions/reviewer.md`) — a real SOLID/SRP/security/idempotency gate, not just "tests green".
