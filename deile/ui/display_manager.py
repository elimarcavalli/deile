...
class DisplayManager:
    ...
    def display_tool_result(
        self, tool_name: str, result: ToolResult, display_policy: Optional[str] = None
    ) -> None:
        """Display tool result according to policy"""

        # Use result's display policy or parameter override
        policy = (
            DisplayPolicy(display_policy) if display_policy else result.display_policy
        )
    ...
    def _display_list_files(self, result: ToolResult) -> None:
        ...
        for file_info in files:
            ...
            dirs[dir_name].append(
                {"name": file_name, "type": file_type, "size": file_size}
            )
    ...
    def _display_generic_tool_result(self, tool_name: str, result: ToolResult) -> None:
        ...
        status_style, status_icon = self._STATUS_DISPLAY.get(
        ...
        if result.error:
            error_panel = Panel(
                str(result.error), title="Error Details", border_style="red"
            )
    ...
    def format_search_results_table(self, results: Dict[str, Any]) -> Table:
        ...
        table = Table(
            title="🔍 Search Results", show_header=True, header_style="blue bold"
        )
        ...
        for match in matches[:20]:  # Limit to 20 for display
            file_name = (
                match["file"].split("/")[-1] if "/" in match["file"] else match["file"]
            )
            line_num = str(match["line_number"])
            match_text = (
                match["match_text"][:80] + "..."
                if len(match["match_text"]) > 80
    ...
    def display_tool_status(
        self, tool_name: str, status: str, message: str = ""
    ) -> None:
