...
class StatusDisplay(UIComponent):
    ...
    def show_loading(self, message: str, details: Optional[str] = None) -> None:
        ...
        status = UIStatus(message=message, is_loading=True, details=details)
        self._current_status = status
        self.renderer.render_status(status)

    def show_progress(
        self, message: str, progress: float, details: Optional[str] = None
    ) -> None:
...
class FileListComponent(UIComponent):
    ...
    def render_file_list(
        self, files: List[str], title: str = "Project Files", max_display: int = 20
    ) -> None:
    ...
    def show_error(
        ...
        if suggestions:
            suggest_msg = UIMessage(
                content="💡 Suggestions:", message_type=MessageType.INFO
            )
            self.renderer.render_message(suggest_msg)

            for i, suggestion in enumerate(suggestions, 1):
                suggestion_msg = UIMessage(
                    content=f"   {i}. {suggestion}", message_type=MessageType.INFO
                )
