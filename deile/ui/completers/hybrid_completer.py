...
class HybridCompleter(Completer):
        ...
        try:
            ...
            for command in self._command_registry.get_enabled_commands():
                if command.name.lower().startswith(fragment_lower):
                    ...
                    if category == "skills":
                        preview = _preview_first_words(
                            getattr(command, "_skill_body", "")
                        )
                        ...
                        cmd_type = (
                            "🤖 LLM" if command.has_prompt_template else "⚡ Direct"
                        )
    ...
    def _get_file_completions(
        ...
        try:
            ...
            if path_fragment.startswith("/") or (
                os.name == "nt" and ":" in path_fragment
            ):
                # Caminho absoluto
    ...
    def _get_contextual_completions(
        ...
        common_starters = [
            ...
            (
                "Adicione",
                "Adicionar um trecho de código ou funcionalidade a um existente",
            ),
            ("Gere", "Gerar uma classe, função, script de build"),
            (
                "Refatore",
                "Refatorar código existente para melhorar clareza e manutenção",
            ),
            (
                "Otimize",
                "Otimizar código PowerScript ou uma query SQL para performance",
            ),
            ...
            (
                "Explique",
                "Explicar o funcionamento de um trecho de código complexo ou legado",
            ),
            ("Documente", "Gerar documentação para uma função, objeto ou processo"),
            # --- Testes ---
            (
                "Gere testes para",
                "Gerar testes unitários ou de integração para o código",
            ),
            (
                "Escreva testes",
                "Criar casos de teste específicos para uma funcionalidade",
            ),
            # --- Banco de Dados (DB2) ---
            ("Crie script SQL DB2", "Gerar DDL ou DML específico para IBM DB2"),
            (
                "Otimize esta query DB2",
                "Analisar e sugerir índices ou reescrita para DB2",
            ),
            ("Gere Stored Procedure", "Criar esqueleto de Stored Procedure para DB2"),
            (
                "Converta SQL para DB2",
                "Traduzir um script SQL de outro dialeto para a sintaxe do DB2",
            ),
            # --- PowerBuilder Específico ---
            ("Converta PowerScript para", "Traduzir lógica de PowerScript para Python"),
            (
                "Crie script OrcaScript",
                "Gerar script de automação de build para PowerBuilder",
            ),
            (
                "Modernize esta tela",
                "Sugerir melhorias de UX/UI para uma janela PowerBuilder",
            ),
            # --- Python Específico ---
            ("Crie API Python com", "Gerar API REST (Flask/FastAPI) para acessar DB2"),
            (
                "Crie script Python",
                "Gerar um script de automação ou processamento de dados",
            ),
            # --- Versionamento (GitLab) ---
            (
                "Crie branch",
                "Sugerir um nome de branch seguindo padrões e gerar comandos",
            ),
            (
                "Revisar MR",
                "Sugerir um checklist e pontos de atenção para revisar um Merge Request",
            ),
            # --- Automação e Ambiente ---
            (
                "Como configurar ambiente",
                "Descrever passos para configurar o ambiente de desenvolvimento local",
            ),
            # --- PowerBuilder ---
            (
                "Analise profundamente este código PowerScript...",
                "para identificar 'code smells', gargalos de performance e pontos de melhoria.",
            ),
            (
                "Crie uma função PowerScript segura e otimizada para...",
                "[descreva aqui a finalidade da função, entradas e saídas esperadas].",
            ),
            (
                "Refatore completamente este objeto PowerBuilder...",
                "para melhorar a manutenibilidade, performance e aplicar princípios SOLID.",
            ),
            (
                "Documente de forma técnica esta função/objeto PowerBuilder...",
                "explicando sua lógica de negócio, parâmetros e dependências.",
            ),
            (
                "Proponha uma modernização de UX/UI para esta janela PowerBuilder...",
                "sugerindo novos controles, layout e melhorias de usabilidade.",
            ),
            # --- IBM DB2 ---
            (
                "Revise e otimize esta query SQL para DB2...",
                "analisando o plano de execução e sugerindo a criação ou ajuste de índices.",
            ),
            (
                "Elabore um script SQL transacional para DB2 que...",
                "[descreva a operação de DDL ou DML, garantindo a atomicidade].",
            ),
            (
                "Desenvolva uma Stored Procedure robusta em DB2 para...",
                "com tratamento completo de exceções, transações e logging de erros.",
            ),
            (
                "Crie a estrutura de tabelas no DB2 para armazenar...",
                "[descreva a entidade de negócio, ex: 'dados de nota fiscal eletrônica'].",
            ),
            # --- Python ---
            (
                "Desenvolva uma API REST em Python com FastAPI para...",
                "[descreva o recurso a ser exposto, ex: 'consultar clientes no DB2'].",
            ),
            (
                "Crie um script Python de automação para...",
                "[descreva a tarefa, ex: 'ler um CSV e inserir os dados no DB2 de forma segura'].",
            ),
            (
                "Escreva testes unitários com Pytest para esta função Python...",
                "garantindo a cobertura dos principais cenários de sucesso e de falha.",
            ),
            # --- Arquitetura e Estratégia ---
            (
                "Desenhe uma arquitetura de microsserviço para substituir...",
                "[descreva a funcionalidade do sistema PowerBuilder a ser modernizada].",
            ),
            (
                "Estruture um pipeline de CI/CD no GitLab para automatizar...",
                "o processo de build e deploy de um projeto PowerBuilder usando OrcaScript.",
            ),
            (
                "Sugira uma estratégia de versionamento e branch no GitLab para...",
                "[descreva o contexto do projeto ou da equipe de desenvolvimento].",
            ),
        ]
