...
class CLI:
    ...
    def print_header(self) -> None:
        ...
        self.console.rule(
            f"[bold #4285F4]DEILE[/][bold #7B68EE] AI AGENT[/] [cyan]v{__version__}[/]",
            style="#4285F4",
        )
        self.console.print(
            "✨ [bold]Agente de IA modular e extensível![/bold]", justify="center"
        )
        self.console.print(
            "Digite 'help' para comandos disponíveis ou 'exit' para sair.",
            justify="center",
        )
    ...
    def print_stats(self, stats: Dict[str, Any]) -> None:
        ...
        if parsers_stats:
            ...
            self.console.print(
                f"  Habilitados: {parsers_stats.get('enabled_parsers', 0)}"
            )
