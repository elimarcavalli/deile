...
class ParserRegistry:
    ...
    def __init__(self):
        ...
        self._parse_cache: Dict[str, Tuple[str, ParseResult]] = (
            {}
        )  # input_hash -> (parser_name, result)
    ...
    def unregister(self, parser_name: str) -> bool:
        ...
        # Remove aliases
        aliases_to_remove = [
            alias for alias, name in self._parser_aliases.items() if name == parser_name
        ]
    ...
    def list_enabled(self) -> List[Parser]:
        ...
        return [
            parser
            for parser in self._parsers_by_priority
    ...
    async def parse(
        ...
        if not input_text or not input_text.strip():
            return ParseResult(
                status=ParseStatus.FAILED, error_message="Empty input provided"
            )
        ...
        if not enabled_parsers:
            return ParseResult(
                status=ParseStatus.FAILED, error_message="No enabled parsers available"
            )
        ...
        for parser in enabled_parsers:
            try:
                if parser.can_parse(input_text):
                    ...
                    if (
                        hasattr(parser, "parse_async")
                        and "working_directory"
                        in parser.parse_async.__code__.co_varnames
                    ):
                        result = await parser.parse_async(
                            input_text, working_directory=working_directory
                        )
                    else:
        ...
        return ParseResult(
            status=ParseStatus.FAILED, error_message="No parser could handle the input"
        )

    async def find_suitable_parsers(
        self, input_text: str
    ) -> List[Tuple[Parser, float]]:
        ...
        for parser in self.list_enabled():
            try:
                ...
                logger.warning(
                    f"Error getting confidence from parser {parser.name}: {e}"
                )
    ...
    def _rebuild_priority_list(self) -> None:
        ...
        self._parsers_by_priority = sorted(
            self._parsers.values(), key=lambda p: p.priority, reverse=True
        )
    ...
    def _clear_parser_cache(self, parser_name: Optional[str] = None) -> None:
        ...
        if parser_name:
            ...
            keys_to_remove = [
                key
                for key, (cached_parser, _) in self._parse_cache.items()
