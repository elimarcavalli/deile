...
class IntelligentFileParser(Parser):
    ...
    async def parse_async(
        self, input_text: str, working_directory: Optional[str] = None
    ) -> ParseResult:
        ...
        try:
            ...
            implicit_files = await self._detect_implicit_files(
                input_text, working_directory
            )

            # 3. Combina os resultados
            combined_result = await self._combine_results(
                explicit_result, implicit_files, input_text, working_directory
            )
    ...
    async def _detect_implicit_files(
        self, input_text: str, working_directory: Optional[str] = None
    ) -> List[Tuple[str, Path]]:
        ...
        for filename in potential_files:
            ...
            if found_path:
                ...
                logger.debug(
                    f"IntelligentFileParser: Auto-detected file {filename} -> {found_path}"
                )
            else:
    ...
    async def _find_file_in_directory(
        self, filename: str, work_dir: Path, max_depth: int = 3
    ) -> Optional[Path]:
            ...
            if max_depth > 0:
                for item in work_dir.iterdir():
                    if item.is_dir() and not item.name.startswith("."):
                        found = await self._find_file_in_directory(
                            filename, item, max_depth - 1
                        )
                        if found:
                            return found
    ...
    async def _combine_results(
        self,
        explicit_result: ParseResult,
        implicit_files: List[Tuple[str, Path]],
        input_text: str,
        ...
        combined_commands = (
            list(explicit_result.commands) if explicit_result.is_success else []
        )
        combined_file_refs = (
            list(explicit_result.file_references) if explicit_result.is_success else []
        )
        uploaded_files = []
        upload_stats = {
        ...
        for filename, file_path in implicit_files:
            ...
            try:
                ...
                upload_result = await self.file_uploader.upload_file_async(
                    str(file_path)
                )
                ...
                if upload_result:
                    ...
                    logger.info(
                        f"Auto-detected and uploaded file: {filename} -> {upload_result.file_uri}"
                    )
        ...
        return ParseResult(
            ...
            metadata={
                "parser": self.name,
                "explicit_files": (
                    len(explicit_result.file_references)
                    if explicit_result.is_success
                    else 0
                ),
