...
from ..infrastructure.google_file_api import (
    GoogleFileUploader,
    UploadError,
    get_file_uploader,
)
        ...
        try:
            ...
            for match in all_matches:
                ...
                try:
                    ...
                    raise ApiFileNotFoundError(
                        f"Arquivo '{file_path}' não encontrado localmente para upload"
                    )
            ...
            action = self._determine_action(
                input_text, [f["original_path"] for f in unique_files]
            )
    ...
    def get_confidence(self, input_text: str) -> float:
        ...
        if total_refs > 0:
            ...
            if any(
                ...
                for keyword in ["read", "write", "show", "create"]
            ):
                base_confidence += 0.1
