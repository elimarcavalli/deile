...
class SecretsScanner:
    ...
    def _load_detection_patterns(
        self,
    ) -> Dict[SecretType, List[Tuple[re.Pattern, float]]]:
        ...
        patterns = {
            SecretType.API_KEY: [
                ...
                (
                    re.compile(
                        r"[aA][pP][iI][kK][eE][yY].*[:=]\s*([A-Za-z0-9_\-]{32,})"
                    ),
                    0.8,
                ),
                (
                    re.compile(
                        r'key.*[:=]\s*["\']([A-Za-z0-9_\-]{20,})["\']', re.IGNORECASE
                    ),
                    0.7,
                ),
            ],
            SecretType.PASSWORD: [
                (
                    re.compile(
                        r'[pP][aA][sS][sS][wW][oO][rR][dD].*[:=]\s*["\']([^"\']{6,})["\']'
                    ),
                    0.8,
                ),
                (re.compile(r'pwd.*[:=]\s*["\']([^"\']{6,})["\']', re.IGNORECASE), 0.7),
                (
                    re.compile(r'passwd.*[:=]\s*["\']([^"\']{6,})["\']', re.IGNORECASE),
                    0.7,
                ),
            ],
            SecretType.TOKEN: [
            ...
            SecretType.GITHUB_TOKEN: [
                ...
                (
                    re.compile(
                        r'github_token.*[:=]\s*["\']([A-Za-z0-9_]{20,})["\']',
                        re.IGNORECASE,
                    ),
                    0.8,
                ),
            ...
            SecretType.GITLAB_TOKEN: [
                ...
                (
                    re.compile(r'GL_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})["\']?'),
                    0.85,
                ),
                (
                    re.compile(
                        r'CI_JOB_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})["\']?'
                    ),
                    0.85,
                ),
            ...
            SecretType.SLACK_TOKEN: [
                ...
                (
                    re.compile(
                        r'slack.*token.*[:=]\s*["\']([^"\']{20,})["\']', re.IGNORECASE
                    ),
                    0.8,
                ),
            ],
            ...
            SecretType.GENERIC_SECRET: [
                ...
                # deilebot control-plane Bearer token (env: DEILE_BOT_AUTH_TOKEN
                ...
                (
                    re.compile(
                        r'DEILE_BOT(?:_CONTROL_PLANE)?_AUTH_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9_\-]{16,})["\']?'
                    ),
                    0.95,
                ),
                ...
                (
                    re.compile(
                        r'(?:DISCORD|DEILE_BOT_DISCORD)_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9_\-]+\.[A-Za-z0-9_\-]+\.[A-Za-z0-9_\-]+)["\']?'
                    ),
                    0.95,
                ),
            ],
    ...
    def _load_whitelist_patterns(self) -> List[re.Pattern]:
        ...
        whitelist = [
            # Common placeholders
            re.compile(
                r"(your_api_key_here|replace_with_your_key|example_key|dummy_token)",
                re.IGNORECASE,
            ),
            re.compile(r"^(lorem ipsum|sample|test|demo|placeholder)$", re.IGNORECASE),
            re.compile(r"^[X\*]+$"),  # Masked values
            re.compile(
                r"^(123456|password|secret|token)$", re.IGNORECASE
            ),  # Common examples
    ...
    def scan_text(
        ...
        for match in sorted(
            matches, key=lambda x: (x.line_number, x.start_pos, -x.confidence)
        ):
    ...
    def scan_directory(
        self, directory_path: Path, file_pattern: str = "*", max_files: int = 1000
        ...
        try:
            for file_path in directory_path.rglob(file_pattern):
                if files_scanned >= max_files:
                    logger.warning(
                        f"Reached max files limit ({max_files}) for scanning"
                    )
                    break
    ...
    def redact_text(
        self, text: str, redaction_char: str = "*"
        ...
        sorted_matches = sorted(
            matches, key=lambda x: (x.line_number, x.start_pos), reverse=True
        )
        ...
        for match in sorted_matches:
            ...
            if 0 <= line_idx < len(lines):
                ...
                lines[line_idx] = (
                    line[: match.start_pos] + replacement + line[match.end_pos :]
                )
    ...
    def redact_file(
        self, file_path: Path, backup: bool = True, redaction_char: str = "*"
    ) -> List[SecretMatch]:
        ...
        try:
            ...
            redacted_content, matches = self.redact_text(
                original_content, redaction_char
            )
