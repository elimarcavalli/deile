...
class ToolRegistry:
    ...
    def unregister(self, tool_name: str) -> bool:
        ...
        # Remove aliases
        aliases_to_remove = [
            alias for alias, name in self._tool_aliases.items() if name == tool_name
        ]
    ...
    async def execute_tool(self, tool_name: str, context: ToolContext) -> ToolResult:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        category_stats = {
            category: len(tools) for category, tools in self._tools_by_category.items()
        }

        # Estatísticas de Function Calling
        tools_with_schemas = sum(
            1 for tool in self._tools.values() if tool.schema is not None
        )
        function_definitions = sum(
            1
            for name, tool in self._tools.items()
            if name in self._enabled_tools
            and tool.get_function_definition() is not None
        )
