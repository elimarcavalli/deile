...
class SearchMatch:
    ...
    context_lines: List[
        str
    ]  # Combined context (before + match + after) ≤ 50 lines total
    match_score: float
...
class SearchTool(SyncTool):
    ...
    def execute_sync(self, context: ToolContext) -> ToolResult:
        ...
        try:
            ...
            max_context_lines = min(
                args.get("max_context_lines", 25), 50
            )  # hard limit: 50 lines per match
    ...
    def _perform_search(
        ...
        files = self._find_searchable_files(
            search_path, file_patterns, exclude_patterns
        )
        ...
        with ThreadPoolExecutor(max_workers=min(8, len(files))) as executor:
            future_to_file = {
                executor.submit(
                    self._search_file_optimized, file_path, pattern, max_context_lines
                ): file_path
        ...
        if len(files) > 1000:
            performance_notes.append(
                f"Limited search to first 1000 files (found {len(files)} total)"
            )
        if total_matches > max_matches:
            performance_notes.append(
                f"Limited results to {max_matches} matches (found {total_matches} total)"
            )
    ...
    def _find_searchable_files(
        self, search_path: Path, file_patterns: List[str], exclude_patterns: List[str]
    ) -> List[Path]:
    ...
    def _search_file_optimized(
        self, file_path: Path, pattern: re.Pattern, max_context_lines: int
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if match_obj:
                    ...
                    context_after_count = min(
                        max_context_lines - context_before_count, len(lines) - line_num
                    )
                    ...
                    if context_before_count + 1 + context_after_count > 50:
                        ...
                        context_before_count = min(
                            context_before_count, total_context // 2
                        )
                        context_after_count = min(
                            context_after_count, total_context - context_before_count
                        )
    ...
    def _calculate_match_score(
        self, line: str, match_obj: re.Match, file_path: Path
    ) -> float:
        ...
        if match_obj.group(0).strip() and (
            (match_obj.start() == 0 or not line[match_obj.start() - 1].isalnum())
            and (match_obj.end() == len(line) or not line[match_obj.end()].isalnum())
        ):
            score += 0.5
