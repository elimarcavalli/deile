...
class IntentMetricsCollector:
    ...
    def __init__(
        ...
        if log_file_path:
            ...
            self.log_file_path = (
                Path.home() / ".deile" / "logs" / "intent_analysis.jsonl"
            )
    ...
    def record_analysis_event(
        ...
        performance_metrics = [
            PerformanceMetric(
                name="analysis_time", value=intent_result.analysis_time, unit="seconds"
            ),
            PerformanceMetric(
    ...
    def _update_real_time_metrics(self, event: IntentAnalysisEvent) -> None:
        ...
        self.real_time_metrics["avg_analysis_time"] = (
            current_avg_time * (total - 1) + new_time
        ) / total
        ...
        self.real_time_metrics["avg_confidence"] = (
            current_avg_conf * (total - 1) + new_conf
        ) / total
        ...
        self.real_time_metrics["avg_complexity"] = (
            current_avg_comp * (total - 1) + new_comp
        ) / total

        # Distribuições
        self.real_time_metrics["intent_distribution"][
            event.intent_result.intent_type.value
        ] += 1
        self.real_time_metrics["category_distribution"][
            event.intent_result.primary_category.value
        ] += 1

        # Efetividade de padrões
        for pattern in event.intent_result.detected_patterns:
            self.real_time_metrics["pattern_effectiveness"][pattern].append(
                event.intent_result.confidence
            )
    ...
    def _check_alerts(self, event: IntentAnalysisEvent) -> None:
        ...
        if (
            event.intent_result.analysis_time
            > self.alert_thresholds["max_analysis_time"]
        ):
            alerts.append(
                {
        ...
        if len(recent_events) >= 50:
            low_confidence_rate = sum(
                1 for e in recent_events if e.intent_result.confidence < 0.5
            ) / len(recent_events)
    ...
    def get_comprehensive_metrics(self) -> Dict[str, Any]:
        ...
        if recent_events:
            ...
            advanced_metrics = {
                "analysis_time_stats": {
                    ...
                    "std_dev": (
                        statistics.stdev(analysis_times)
                ...
                "complexity_stats": {
                    ...
                    "std_dev": (
                        statistics.stdev(complexities) if len(complexities) > 1 else 0
                    ),
                },
            }
        ...
        for pattern, confidence_scores in self.real_time_metrics[
            "pattern_effectiveness"
        ].items():
            if confidence_scores:
                pattern_effectiveness[pattern] = {
                    ...
                    "effectiveness_score": statistics.mean(confidence_scores)
    ...
    def _calculate_period_trends(
        self, events: List[IntentAnalysisEvent], period_seconds: int
    ) -> List[IntentTrend]:
        ...
        for intent_type, intent_events in intent_groups.items():
            ...
            if recent_half and older_half:
                recent_avg = statistics.mean(
                    [e.intent_result.confidence for e in recent_half]
                )
                older_avg = statistics.mean(
                    [e.intent_result.confidence for e in older_half]
                )
            ...
            trend = IntentTrend(
                period=(
                    f"{period_seconds//3600}h"
                    if period_seconds < 86400
                    else f"{period_seconds//86400}d"
                ),
    ...
    def _assess_system_health(self) -> Dict[str, Any]:
        ...
        if self.real_time_metrics["total_analyses"] > 0:
            failure_rate = (
                self.real_time_metrics["failed_analyses"]
                / self.real_time_metrics["total_analyses"]
            )
    ...
    def _generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if len(self.events) > 100:
            unique_inputs = len(
                set(hash(e.user_input) for e in list(self.events)[-100:])
            )
    ...
    def clear_old_data(self) -> None:
        ...
        if old_count != new_count:
            logger.info(
                f"Cleaned {old_count - new_count} old events from metrics storage"
            )
