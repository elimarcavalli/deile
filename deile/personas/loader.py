...
class PersonaLoader:
    ...
    async def load_persona(self, config: PersonaConfig) -> BaseAutonomousPersona:
        ...
        try:
            ...
            if errors:
                logger.warning(
                    f"Persona '{config.name}' tem erros de validação: {errors}"
                )

            logger.debug(
                f"Persona '{config.name}' carregada com classe {persona_class.__name__}"
            )
            return persona
    ...
    async def _load_persona_class_by_name(
        self, class_name: str
    ) -> Type[BaseAutonomousPersona]:
    ...
    async def _get_generic_persona_class(self) -> Type[BaseAutonomousPersona]:
        ...
        class GenericPersona(BaseAutonomousPersona):
            ...
            async def build_system_instruction(
                ...
                if md_instruction:
                    logger.info(
                        f"Using MD instruction for persona {self.config.persona_id}"
                    )
                    base = md_instruction
                else:
                    logger.info(
                        f"Using YAML instruction for persona {self.config.persona_id}"
                    )
                ...
                if context:
                    ...
                    if "working_directory" in context:
                        context_info.append(
                            f"Diretório de trabalho: {context['working_directory']}"
                        )
    ...
    def register_persona_class(
        self, persona_id: str, persona_class: Type[BaseAutonomousPersona]
    ) -> None:
        ...
        logger.debug(
            f"Classe de persona registrada: {persona_id} -> {persona_class.__name__}"
        )
            ...
            for py_file in search_path.glob("*.py"):
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (
                            name != "BaseAutonomousPersona"
                            and issubclass(obj, BaseAutonomousPersona)
