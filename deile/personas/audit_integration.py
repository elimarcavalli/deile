...
class PersonaErrorAuditLogger:
    ...
    async def log_persona_error(
        ...
        try:
            ...
            try:
                logger.critical(
                    f"AUDIT SYSTEM FAILURE - Error: {error.error_code}, Context: {context.error_id}, Audit Error: {audit_error}"
                )
            except Exception:
                ...
                print(
                    f"CRITICAL: Audit system completely failed - {audit_error}",
                    file=sys.stderr,
                )
    ...
    async def log_recovery_attempt(
        ...
        try:
            ...
            event_type = (
                AuditEventType.PERSONA_RECOVERY_SUCCESS
                if success
                else AuditEventType.PERSONA_RECOVERY_FAILURE
            )
            severity = SeverityLevel.INFO if success else SeverityLevel.WARNING
    ...
    async def get_persona_audit_summary(
        self, persona_id: Optional[str] = None, time_range_hours: int = 24
    ) -> Dict[str, Any]:
...
async def log_persona_switch(
    from_persona: str, to_persona: str, success: bool, **kwargs
) -> None:
    """Convenience function for logging persona switches"""
    await get_persona_audit_logger().log_persona_switch(
        from_persona, to_persona, success, **kwargs
    )


async def log_recovery_attempt(
    error: PersonaError, context: ErrorContext, recovery_action: str, success: bool
) -> None:
    """Convenience function for logging recovery attempts"""
    await get_persona_audit_logger().log_recovery_attempt(
        error, context, recovery_action, success
    )
