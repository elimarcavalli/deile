...
class PersonaConfig:
    ...
    async def load_from_config_manager(
        cls, persona_id: str, config_manager: ConfigManager = None
    ) -> "PersonaConfig":
    ...
    @classmethod
    def from_dict(
        cls, persona_id: str, data: Dict[str, Any], config_manager: ConfigManager = None
    ) -> "PersonaConfig":
    ...
    async def reload(self) -> None:
        ...
        try:
            updated_config = await self.config_manager.get_persona_config(
                self.persona_id
            )
            ...
            if updated_config:
                ...
                self.logger.debug(
                    f"Reloaded configuration for persona {self.persona_id}"
                )
    ...
    def __str__(self) -> str:
        return (
            f"PersonaConfig({self.persona_id}, {len(self.capabilities)} capabilities)"
        )
