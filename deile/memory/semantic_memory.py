...
class SemanticMemory:
    ...
    async def search_knowledge(
        ...
        for knowledge in self._knowledge_base[-max_results:]:  # Pega os mais recentes
            content = str(knowledge.get("user_input", "")) + str(
                knowledge.get("agent_response", "")
            )
            if query_lower in content.lower():
                results.append(
                    {"content": content, "score": 1.0, "metadata": knowledge}
                )

        return results

    async def store_correction(
        self, interaction_id: str, correction_data: Dict[str, Any]
    ) -> None:
        """Armazena correção"""
        correction = {
            "type": "correction",
