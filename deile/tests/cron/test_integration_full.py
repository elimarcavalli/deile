...
class TestCronEndToEnd:
    ...
    async def test_session_id_encodes_entry_id(self, tmp_path):
        ...
        _runner = CronRunner(
            store,
            fire_callback=make_fire_callback(
                lambda: asyncio.coroutine(lambda: agent)()
            ),
        )
        ...
        async def recording_cb(entry: CronEntry) -> str:
            ...
            captured["session_id"] = agent.process_input.await_args.kwargs.get(
                "session_id"
            )
            return result
    ...
    async def test_agent_provider_error_does_not_crash_runner(self, tmp_path):
        ...
        assert loaded is not None
        assert (
            "RuntimeError" in (loaded.last_result or "")
            or "error" in (loaded.last_result or "").lower()
        )
    ...
    async def test_summary_truncated_at_max_chars(self, tmp_path):
        ...
        runner = CronRunner(
            store, fire_callback=make_fire_callback(provider, max_summary_chars=100)
        )
        await runner.tick()
