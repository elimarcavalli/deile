...
from deile.orchestration.workflow_executor import WorkflowExecutor, WorkflowStep
...
def _make_task(
    task_id="t001", title="Test Task", status=TaskStatus.TODO, metadata=None
):
...
def _make_task_manager(
    ...
    mgr.add_task_to_list = AsyncMock(
        side_effect=lambda **kw: _make_task(
            task_id="t001",
            title=kw.get("title", "step"),
    ...
    mgr.get_task_list_status = AsyncMock(
        return_value={
            "id": "list1",
            "title": "Test Workflow",
            "active": True,
...
async def test_task_metadata_persisted_in_add_task():
    ...
    step = WorkflowStep(
        action="list_files", params={"path": "."}, description="List files", timeout=60
    )
...
async def test_validation_action_passes_when_no_failures():
    ...
    completed = _make_task(
        task_id="prev", title="Previous Step", status=TaskStatus.COMPLETED
    )
...
async def test_validation_action_fails_when_previous_step_failed():
    ...
    failed_step = _make_task(
        task_id="prev", title="Broken Step", status=TaskStatus.FAILED
    )
