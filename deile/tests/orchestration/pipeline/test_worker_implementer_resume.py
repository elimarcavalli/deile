...
async def test_dispatch_records_in_ledger_on_failure(tmp_path):
    ...
    impl = WorkerImplementer(client=client, endpoint_override="http://x", ledger=ledger)
    out = await impl._dispatch(
        "do work",
        channel_id="pipeline-pr-100",
        stage="pr_review",
        ledger_key="pr:100",
    )
...
async def test_resume_consults_resume_info_when_resume_true(tmp_path):
    ...
    client = _fake_client(
        dispatch_response={
            "ok": True,
            "summary": "done resumed",
            "task_id": "prevTaskId123456",
            "session_id": "sess-X",
        },
...
async def test_resume_fallback_fresh_when_workdir_lost(tmp_path):
    ...
    payload = (
        client.dispatch.await_args.kwargs.get("payload")
        or client.dispatch.await_args.args[0]
    )
...
async def test_resume_records_worker_kind_based_on_url(tmp_path):
    ...
    impl = WorkerImplementer(
        client=client, endpoint_override="http://claude-worker:8767", ledger=ledger
    )
    await impl._dispatch(
        "x",
        channel_id="pipeline-pr-100",
...
async def test_records_worker_kind_for_cli_fleet_worker(tmp_path):
    ...
    impl = WorkerImplementer(
        client=client, endpoint_override="http://opencode-worker:8771", ledger=ledger
    )
    await impl._dispatch(
        "x",
        channel_id="pipeline-pr-200",
        stage="implement",
        ledger_key="pr:200",
    )
...
def test_worker_kind_from_url_helper():
    ...
    assert (
        _worker_kind_from_url("http://opencode-worker:8771/v1/dispatch") == "opencode"
    )
