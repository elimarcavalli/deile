...
from deile.core.models.error_mapping import (
...
def test_classify_http_error_extra_markers():
    assert (
        classify_http_error(400, "", "prompt is too long", ("prompt is too long",))
...
def test_classify_provider_error_dict_body_extractor():
    exc = _FakeExc(
        status_code=400, body={"type": "context_length_exceeded", "message": "too big"}
    )
...
def test_classify_provider_error_non_dict_body_falls_back_to_str():
    ...
    exc = _FakeExc(
        "the maximum context length was exceeded", status_code=400, body="not-a-dict"
    )
    assert classify_provider_error(exc, _anthropic_like) == "context_length_exceeded"
...
def test_build_error_envelope_dict_body():
    exc = _FakeExc("bad key", status_code=401, body={"error": "x"}, request_id="req_9")
