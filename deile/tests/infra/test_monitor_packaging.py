...
def server_src() -> str:
    return (_REPO / "infra" / "k8s" / "monitor_command_server.py").read_text(
        encoding="utf-8"
    )
...
def manifest() -> str:
    return (
        _REPO / "infra" / "k8s" / "manifests" / "55-deile-monitor-deployment.yaml"
...
def test_manifest_runs_command_server_via_args_preserving_tini(manifest):
    ...
    assert container.get("command") in (
        None,
        [],
...
def test_server_runs_flattened_monitor_tick(server_src):
    ...
    assert (
        "/app/monitor_tick.py" in server_src
...
def test_manifest_has_liveness_reflecting_tick(manifest):
    ...
    assert (
        live and live["httpGet"]["path"] == "/v1/health"
...
def test_manifest_bot_mounts_monitor_bearer():
    ...
    bot = (_REPO / "infra" / "k8s" / "manifests" / "20-bot-deployment.yaml").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        mounts.get("monitor-bearer") == "/run/secrets/bot/monitor"
