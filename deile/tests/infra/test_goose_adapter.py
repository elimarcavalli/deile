...
def test_build_argv_resume_reopens_named_session(adapter, brief):
    ...
    resume = base.ResumeCtx(
        session_id="0123456789abcdef", prev_task_id="0123456789abcdef"
    )
    argv = adapter.build_argv(
        brief_path=brief,
        model=None,
...
def test_parse_output_single_object(adapter):
    wr = adapter.parse_output(
        stdout=json.dumps({"result": "tarefa concluída"}),
        stderr="",
        rc=0,
    )
...
def test_parse_output_object_error(adapter):
    wr = adapter.parse_output(
        stdout=json.dumps({"error": "provider indisponível"}),
        stderr="",
        rc=0,
    )
