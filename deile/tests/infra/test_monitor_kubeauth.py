...
def test_write_kubeconfig_uses_tokenfile_not_inline_token(core, tmp_path):
    ...
    core.write_incluster_kubeconfig(str(kc), "https://kubernetes.default.svc:443", sa_dir=str(sa))
...
def test_resolve_incluster_none_when_all_probes_fail(core, tmp_path):
    sa = _fake_sa(tmp_path)
    kc = tmp_path / "kubeconfig"
    assert core.resolve_incluster_kube(FakeRunner({"version": (1, "")}), str(kc), sa_dir=str(sa)) is None
...
def test_kubectl_uses_kubeconfig_when_set(core, vig):
    ...
    ctx = vig.MonitorContext(
        run=runner, emitter=core.Emitter("/dev/null", core.TickFlags()),
        notifier=None, state=core.default_state(), flags=core.TickFlags(),
        now=_utc(2026, 6, 2, 11, 0, 0), repo="r", namespace="deile",
...
async def test_run_tick_authenticates_via_kubeconfig_when_sa_present(tick, tmp_path, capsys):
    sa = _fake_sa(tmp_path)
    sd = tmp_path / "state"
    ...
    await tick.run_tick(
        ...
        sa_dir=str(sa), kubeconfig_path=str(tmp_path / "kc"),
    )
