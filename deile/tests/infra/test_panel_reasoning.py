...
class TestDispatchMatrixReasoningColumn:
    ...
    def test_picker_options_contextual(self):
        ...
        v = P.DispatchMatrixView(data=None)
        claude = v._reasoning_picker_options(
            worker="claude-worker", model="anthropic:claude-opus-4-8"
        )
        assert claude[1:] == [
            "low",
            "medium",
        ...
        openai = v._reasoning_picker_options(
            worker="deile-worker", model="openai:gpt-5.4"
        )
        assert "none" in openai and "xhigh" in openai
        gemini = v._reasoning_picker_options(
            worker="deile-worker", model="gemini:gemini-2.5-pro"
        )
        assert "off" in gemini
        deepseek = v._reasoning_picker_options(
            worker="deile-worker", model="deepseek:deepseek-v4-pro"
        )
