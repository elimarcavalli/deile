...
def test_codex_parses_token_count_cumulative_delta(fta, tmp_path):
    ...
    _write_progress(
        ...
        [
            ...
            json.dumps(
                {"type": "turn_context", "turn_context": {"model": "gpt-5.1-codex"}}
            ),
            json.dumps(
                {
                    "type": "token_count",
...
def test_claude_parses_jsonl_with_dedup(fta, tmp_path):
    ...
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env={"FLEET_KIND": "claude", "PATH": "/usr/bin:/bin", **env},
    )
    assert proc.returncode == 0, proc.stderr
...
def test_deile_parses_usage_sqlite(fta, tmp_path):
    ...
    con.commit()
    con.close()
    sessions = _run_parser(
        fta, "deile", tmp_path, env_extra={"FLEET_DEILE_DB": str(db)}
    )
    assert len(sessions) == 1
    s = sessions[0]
...
def test_enrich_costs_per_model_and_native_preference(fta):
    ...
    sessions = [
        ...
        # qwen sem custo nativo → estimado via tabela.
        {
            ...
            "native_cost": None,
            "models": {
                "qwen3-coder-plus": {"in": 1_000_000, "out": 0, "cc": 0, "cr": 0}
            },
            "first_ts": None,
            "last_ts": None,
            "mtime": None,
    ...
    s = [
        {
            ...
            "native_cost": None,
            "models": {
                "claude-sonnet-4-6": {"in": 1_000_000, "out": 0, "cc": 0, "cr": 0}
            },
            "first_ts": None,
            "last_ts": None,
            "mtime": None,
...
def test_collector_declared_price_overrides_table(fta):
    ...
    cost = coll.cost_for_model(
        {"in": 1_000_000, "out": 0, "cc": 0, "cr": 0}, "some-exotic-model"
    )
        ...
        [
            {
                ...
                "native_cost": 0.01,
                "models": {
                    "deepseek/deepseek-v4-pro": {"in": 100, "out": 20, "cc": 0, "cr": 0}
                },
                "first_ts": "2026-06-01T10:00:00Z",
                "last_ts": "2026-06-01T10:05:00Z",
                "mtime": 1_700_000_000.0,
                "brief": "implemente algo",
            },
            {
                "worker": "qwen",
                "task_id": "t2",
                ...
                "native_cost": None,
                "models": {
                    "qwen3-coder-plus": {"in": 5000, "out": 1000, "cc": 0, "cr": 0}
                },
                "first_ts": None,
                "last_ts": None,
                "mtime": None,
...
def test_collect_all_central_works_without_kubectl(fta, tmp_path, monkeypatch):
    ...
    sessions = fta.collect_all(
        "", "deile", fta.fleet_worker_kinds(), collectors, source="central"
    )
    assert len(sessions) == 1
...
def test_collect_all_both_central_supersedes_pod_session(fta, tmp_path, monkeypatch):
    ...
    def _fake_pods(_kubectl, _ns, _kinds, _since=0.0):
        return [
            ...
            },
            {
                ...
                "source": "jsonl",
                "models": {
                    "claude-sonnet-4-6": {"in": 100, "out": 20, "cc": 0, "cr": 0}
                },
                "native_cost": None,
                "first_ts": None,
                "last_ts": None,
    ...
    sessions = fta.collect_all(
        "kc", "deile", fta.fleet_worker_kinds(), collectors, source="both"
    )
