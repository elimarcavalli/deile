...
def _ctx(core, vig, runner, now):
    ...
    notifier = SimpleNamespace(
        sent=sent,
        notify=lambda fp, sev, title, body, **kw: (
            sent.append((fp, sev, title)) or True
        ),
    )
    ...
    issues = [
        {
            ...
            "updated_at": "2026-06-02T05:00:00Z",
            "labels": [{"name": "~workflow:aguardando_stakeholder"}],
            "assignees": [{"login": "x"}],
        }
    ]
    ...
    pulls = [
        {
            "number": 501,
            "title": "auto pr",
            "head": {"ref": "auto/issue-440"},
            "updated_at": "2026-06-02T10:00:00Z",
        }
    ]
    comments = [
        {"body": "tentativa attempt 2/3 falhou", "created_at": "2026-06-02T10:30:00Z"}
    ]
    runner = FakeRunner(
        {
...
def test_v8_collects_human_promise_as_candidate(core, vig):
    now = _utc(2026, 6, 2, 11, 0, 0)
    closed = [
        {"number": 445, "title": "X", "body": "", "closed_at": "2026-06-02T09:00:00Z"}
    ]
    comments = [
        {
            "id": 700,
            "body": "vou abrir uma issue para tratar o resto",
            "user": {"login": "elimarcavalli"},
...
def test_v8_skips_bot_author(core, vig, capsys):
    now = _utc(2026, 6, 2, 11, 0, 0)
    closed = [
        {"number": 446, "title": "X", "body": "", "closed_at": "2026-06-02T09:00:00Z"}
    ]
    comments = [
        {
            "id": 701,
            "body": "vou abrir uma issue",
            "user": {"login": "deile-one[bot]"},
            "created_at": "2026-06-02T09:30:00Z",
        }
    ]
    runner = FakeRunner(
...
def test_v8_skips_already_fingerprinted(core, vig):
    now = _utc(2026, 6, 2, 11, 0, 0)
    closed = [
        {"number": 447, "title": "X", "body": "", "closed_at": "2026-06-02T09:00:00Z"}
    ]
    comments = [
        {
            "id": 702,
            "body": "follow-up: tratar isso",
            "user": {"login": "human"},
            "created_at": "2026-06-02T09:30:00Z",
        }
    ]
