...
def test_cluster_status_screen_renders():
    ...
    data = ClusterStatusData(
        ...
        backlog=[
            {"kind": "issue", "number": 347, "title": "live panel",
             "age_seconds": 300, "why_eligible": "new"},
        ],
        recent_events=[
            {"ts": time.time() - 60, "event_type": "merged",
...
def test_cluster_status_screen_adapts_width_80():
    ...
    data = ClusterStatusData(
        pipeline_status={"uptime_seconds": 60, "ticks_total": 1,
                         "errors_total": 0,
                         "last_tick_at": time.time(),
                         "next_tick_at": time.time(), "pods_seen": {},
                         "now": time.time()},
        backlog=[{"kind": "issue", "number": 1, "title": "x",
                  "age_seconds": 10, "why_eligible": "new"}],
        recent_events=[],
        ledger={},
        api_errors=[],
    )
...
def test_cluster_status_screen_adapts_width_200():
    ...
    data = ClusterStatusData(
        ...
        backlog=[],
        recent_events=[
            {"ts": time.time(), "event_type": "merged",
             "summary": "PR #346 merged"},
        ],
        ledger={},
        api_errors=[],
    ...
    data = LiveSessionData(
        ...
        chat={
            "turns": [
                ...
                {"role": "assistant", "content": "Reading files first.",
                 "ts": now - 160},
                {"role": "tool", "tool_name": "Bash",
                 "tool_input": {"command": "gh pr view 346"},
                 "ts": now - 158, "in_progress": True},
            ],
        },
        api_errors=[],
    )
...
def test_history_screen_renders_chronological_list():
    ...
    now = time.time()
    sessions = [
        {"task_id": "be75a424e2b06dc6", "stage": "pr_review",
         "branch": "auto/issue-345", "last_completed_at": now - 600,
         "last_duration_seconds": 842, "last_total_cost_usd": 1.47,
         "last_is_error": False, "attempt": 1},
        {"task_id": "dcfd09d75c0c5ea1", "stage": "implement",
         "branch": "auto/issue-345", "last_completed_at": now - 1200,
         "last_duration_seconds": 105, "last_total_cost_usd": 0.0,
         "last_is_error": True, "attempt": 2},
    ]
    out = render_to_string(HistoryScreen(), HistoryData(sessions, []),
                           width=120)
...
def test_history_screen_marks_resumed_attempts():
    ...
    sessions = [
        {"task_id": "x", "stage": "impl", "branch": "b",
         "last_completed_at": time.time(), "last_duration_seconds": 10,
         "last_total_cost_usd": 0.01, "last_is_error": True, "attempt": 3},
    ]
