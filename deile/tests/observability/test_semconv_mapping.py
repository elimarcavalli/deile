...
def test_normalize_https_with_git_suffix():
    assert _normalize_repo_url("https://github.com/owner/repo.git") == "https://github.com/owner/repo"
...
def test_git_commit_span_has_deile_and_semconv_attrs(in_memory_exporter):
    ...
    emit_git_commit(tid, repo="git@github.com:owner/myrepo.git", sha="abc123", status="ok")
...
def test_git_push_span_has_deile_and_semconv_attrs(in_memory_exporter):
    ...
    emit_dispatch_received(tid, session_id="s1", model="m", branch="main")
    emit_git_push(tid, repo="https://github.com/owner/myrepo.git", branch="feat/x", status="ok")
...
def test_forge_pr_open_span_has_semconv_attrs(in_memory_exporter):
    ...
    emit_dispatch_received(tid, session_id="s1", model="m", branch="main")
    emit_forge_pr_open(tid, repo="https://github.com/owner/repo.git", pr_number=42, status="open")
...
def test_forge_pr_review_span_has_semconv_attrs(in_memory_exporter):
    ...
    emit_dispatch_received(tid, session_id="s1", model="m", branch="main")
    emit_forge_pr_review(tid, repo="git@gitlab.com:owner/repo.git", pr_number=7, status="approved")
...
def test_semconv_disabled_no_vcs_attrs(in_memory_exporter, monkeypatch):
    ...
    emit_dispatch_received(tid, session_id="s1", model="m", branch="main")
    emit_git_commit(tid, repo="git@github.com:owner/repo.git", sha="deadbeef", status="ok")
