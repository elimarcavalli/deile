...
class CommandResult:
    ...
    @classmethod
    def success_result(
        cls, content: Union[str, Any], content_type: str = "text", **metadata
    ) -> "CommandResult":
    ...
    @classmethod
    def error_result(
        cls, message: str, error: Optional[Exception] = None, **metadata
    ) -> "CommandResult":
...
class SlashCommand(ABC):
    ...
    def get_prompt_for_llm(self, args: str, **kwargs) -> Optional[str]:
        ...
        try:
            return self.config.prompt_template.format(args=args, **kwargs)
