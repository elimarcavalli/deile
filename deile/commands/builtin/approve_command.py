...
class ApproveCommand(DirectCommand):
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        if len(parts) < 2:
            raise CommandError(
                "approve command requires plan ID and step ID: /approve <plan_id> <step_id> [yes|no]"
            )
        ...
        if len(parts) > 2:
            ...
            if approval_response in ["no", "n", "reject", "deny", "false"]:
                ...
                raise CommandError(
                    f"Invalid approval response: {approval_response}. Use 'yes' or 'no'"
                )
    ...
    async def _show_pending_approvals(self) -> CommandResult:
        ...
        table = Table(
            title=f"⚠️ Pending Approvals ({len(pending_approvals)} steps)",
            show_header=True,
    ...
    async def _approve_step(
        self, plan_id: str, step_id: str, approved: bool
    ) -> CommandResult:
        ...
        if step.status != StepStatus.REQUIRES_APPROVAL:
            raise CommandError(
                f"Step '{step_id}' is not waiting for approval (status: {step.status.value})"
            )
        ...
        if approved and step.params:
            content_lines.extend(["", "**Parameters:**"])
