...
class PatchCommand(DirectCommand):
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        return await self._generate_patch(
            plan_id, output_format, output_path, include_artifacts
        )
    ...
    async def _list_patches(self) -> CommandResult:
        ...
        for patch_file in patch_files:
            ...
            table.add_row(patch_file.name, plan_id, created, size, action_text)
    ...
    async def _generate_patch(
        ...
        if plan.status not in ["completed", "failed"]:
            raise CommandError(
                f"Cannot generate patch for plan with status '{plan.status.value}'. Only completed or failed plans can be patched."
            )
        ...
        if not changes["has_changes"]:
            return CommandResult.success_result(
                Panel(
                    Text(
                        f"Plan '{plan_id}' made no changes that can be patched.\n\nThis plan may have only performed read operations.",
                        style="yellow",
                    ),
        ...
        patch_content = await self._create_patch_content(
            plan, changes, output_format, include_artifacts
        )

        # Determine output path
        if not output_path:
            output_path = (
                ensure_patches_dir() / f"plan_{plan_id}_{export_timestamp()}.patch"
            )
        else:
        ...
        try:
            ...
            return await self._format_patch_result(
                plan, changes, output_path, output_format, include_artifacts
            )
    ...
    async def _create_patch_content(
        self, plan, changes: Dict[str, Any], output_format: str, include_artifacts: bool
    ) -> str:
    ...
    async def _format_patch_result(
        self,
        plan,
        ...
        if changes["file_changes"]:
            ...
            for file_change in changes["file_changes"][:5]:  # Show first 5
                content_lines.append(
                    f"  {file_action_emoji(file_change['action'])} {file_change['path']}"
                )

            if len(changes["file_changes"]) > 5:
                content_lines.append(
                    f"  ... and {len(changes['file_changes']) - 5} more files"
                )
