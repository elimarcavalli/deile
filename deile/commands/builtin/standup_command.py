...
from ._standup_collectors import (
    StandupData,
...
async def generate_narrative(prompt: str) -> str:
    ...
    response = await provider.generate(
        messages,
        system_instruction="Você é um assistente técnico que gera resumos de standup em PT-BR.",
    )
...
class StandupCommand(DirectCommand):
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        return CommandResult.success_result(
            Panel(
                Text(narrative),
                title=f"📰 DEILE — Standup das últimas {since_spec}",
                border_style="blue",
            ),
            "rich",
            **metadata,
