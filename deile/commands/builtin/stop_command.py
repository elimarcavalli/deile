...
class StopCommand(DirectCommand):
    ...
    async def _show_stoppable_plans(self) -> CommandResult:
        ...
        table = Table(
            title=f"⏹️ Plans Available to Stop ({len(stoppable_plans)} plans)",
            show_header=True,
    ...
    async def _stop_plan(self, plan_id: str, force: bool = False) -> CommandResult:
        ...
        if plan.status not in [PlanStatus.RUNNING, PlanStatus.PAUSED]:
            raise CommandError(
                f"Plan '{plan_id}' is not running (status: {plan.status.value})"
            )
        ...
        if not success:
            raise CommandError(
                f"Failed to stop plan '{plan_id}'. Plan may not be running."
            )
        ...
        if status_before and status_before.get("current_steps"):
            ...
            content_lines.extend(["", "**Interrupted Steps:**"])

            for step in current_steps:
                step_status = (
                    "🔄 Running"
                    if step["status"] == "running"
                    else "⚠️ Waiting approval"
                )
