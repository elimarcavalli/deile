...
def load_skills_config(path: Optional[Path] = None) -> SkillsConfig:
    ...
    - **Missing file** → defaults (``enabled=True``). The subsystem ships with
      sensible defaults — absence of override is not a reason to disable.
    - **Unreadable file** → defaults + WARNING. Same reasoning.
    - **Malformed YAML** or **non-mapping root** → ``enabled=False`` + WARNING.
      The user wrote a config and got it wrong; disabling surfaces the mistake
      rather than silently ignoring intent.
    """
